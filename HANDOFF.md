...
### 2026-07-25 (fable5監査対応)
- 戦略期待値を貸玉・交換率・持ち玉比率対応へ変更し、等価/実質ボーダーを分離
- P-EVIDENCEとDeltaの日次観測、判定、時間配分、日付規約を共通化
- 算定不能な締め確率をnull化し、遷移件数の根拠を表示
- 現金上限の残額・残り回転・停止目安を戦略/実戦画面へ追加
- 未稼働の島信号と、翌日予測バックテスト・校正候補を追加
- 未来日の実戦混入、最新読取失敗時の旧予測復活、当り0・差玉プラスを防止
- 監査計画: `docs/fable5-audit-implementation-plan.md`
- 最新main上の全412テスト、ESLint、本番ビルド、実ブラウザ確認を完了。`logic.js` は未変更
