...
================================================================ */
export function HistoryTab({ jpLog, sesLog, _pushJP, delJPLast, delSesLast, S, ev }) {
    ...
    const machineRushRounds = useMemo(() => {
        const machine = searchMachines(S.machineName, S.customMachines)[0];
...
export function CalendarTab({ S, onReset }) {
    ...
    if (selectedArchiveId && selectedArchiveId !== prevSelectedRef.current) { // eslint-disable-line react-hooks/refs
        const target = archives.find(ar => ar.id === selectedArchiveId);
        if (target) {
            prevSelectedRef.current = selectedArchiveId; // eslint-disable-line react-hooks/refs
    ...
    } else if (!selectedArchiveId && prevSelectedRef.current) { // eslint-disable-line react-hooks/refs
        prevSelectedRef.current = null; // eslint-disable-line react-hooks/refs
    }
        ...
        return (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                <div style={{ padding: "12px 14px", flexShrink: 0, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <button className="b" onClick={() => { setSelectedArchiveId(null); setDelConfirm(null); }} style={{
...
export function SettingsTab({ s, onReset }) {
    ...
    const normalizedStores = (s.stores || []).map(st =>
        typeof st === "string"
            ? { id: Date.now() + Math.random(), name: st, address: "", rentBalls: 250, exRate: 250, memo: "", chodama: 0 } // eslint-disable-line react-hooks/purity
