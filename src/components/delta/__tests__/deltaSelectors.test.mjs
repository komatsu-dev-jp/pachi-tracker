...
test("mergeTaiData: 校正済みの未読取線は低稼働・当り0と一致した時だけ0玉候補にする", () => {
  const missingRow = (num, calibrationSource = "panel") => ({
    num,
    val: null,
    rank: null,
    status: "failed",
    reasonCodes: ["missing-series"],
    calibration: { source: calibrationSource, quality: 0.9 },
  });
  const source = [
    missingRow("818"),
    missingRow("819"),
    missingRow("820"),
    missingRow("821", "image-median"),
  ];
  const imported = [
    { num: "818", normalSpins: 17, totalStarts: 0 },
    { num: "819", normalSpins: 151, totalStarts: 0 },
    { num: "820", normalSpins: 17, totalStarts: 1 },
    { num: "821", normalSpins: 17, totalStarts: 0 },
  ];

  const { rows: merged } = mergeTaiData(source, imported);
  assert.strictEqual(merged[0].val, 0);
  assert.strictEqual(merged[0].rank, "C");
  assert.strictEqual(merged[0].status, "review", "表の一致だけでは自動確定しない");
  assert.strictEqual(merged[0].valueSource, "low-activity-evidence");
  assert.deepStrictEqual(merged[0].reasonCodes, [
    "zero-length-series",
    "table-low-activity",
    "low-confidence",
  ]);
  assert.strictEqual(isResolvedDeltaRow(merged[0]), false);

  for (const row of merged.slice(1)) {
    assert.strictEqual(row.val, null, `${row.num}: 条件外は0玉へ変換しない`);
    assert.strictEqual(row.status, "failed");
  }

  const confirmed = updateDeltaReview(merged[0], {
    value: 0,
    confirmed: true,
...
test("validateDeltaRows: 一点値を捏造せずbounded範囲のまま保存可能にする", () => {
  const [bounded] = attachClippedDeltaRanges([{
    num: "548",
    val: 28500,
    rank: "SS",
    status: "review",
...
test("validateDeltaRows: 見かけだけのbounded範囲は保存可能にしない", () => {
  const fake = {
    ...
    reviewConfirmed: false,
    calibration: { source: "image-median" },
    reasonCodes: ["endpoint-clipped-top", "clipped-series"],
    boundaryObservation: { kind: "censored-top", boundary: "top", value: 30000, exact: false },
    deltaRange: { kind: "censored-top", boundary: "top", boundaryValue: 30000, observedCandidate: 28500, lower: null, upper: null, source: "graph-boundary", exact: false },
  };
  assert.equal(validateDeltaRows([fake]).valid, false);
});

test("updateDeltaReview: 手動の正確値から安全な範囲記録へ戻せる", () => {
  const [bounded] = attachClippedDeltaRanges([{
    num: "548",
    val: 28500,
    status: "review",
  ...
  const exact = updateDeltaReview(bounded, { value: 45500, confirmed: true });
  assert.equal(exact.reviewConfirmed, true);

  const restored = updateDeltaReview(exact, { restoreBounded: true });
  assert.equal(restored.status, "bounded");
  assert.equal(restored.valueSource, "bounded-range");
  assert.equal(restored.val, null);
  assert.equal(validateDeltaRows([restored]).valid, true);
});
