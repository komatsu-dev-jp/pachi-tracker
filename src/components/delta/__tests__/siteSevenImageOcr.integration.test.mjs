...
const expectedTableRows = readFileSync(
  fileURLToPath(new URL("./fixtures/site-seven-table-52.expected.csv", import.meta.url)),
  "utf8",
).trim().split(/\r?\n/u).slice(1).map((line) => line.split(",").map(Number));
const expectedCumulativeStarts = expectedTableRows.map((row) => row[1]);
const expectedFirstHitCounts = expectedTableRows.map((row) => row[3]);
...
test("実画像52台: 台番号・通常回転・大当たり回数を全件一致させる", async () => {
  ...
  assert.deepEqual(result.rows.map((row) => Number(row.cumulativeStarts)), expectedCumulativeStarts);
  assert.deepEqual(result.rows.map((row) => Number(row.firstHitCount)), expectedFirstHitCounts);
  ...
  assert.ok(result.rows.every((row) => row.cumulativeStartsAccepted));
  assert.ok(result.rows.every((row) => row.firstHitCountAccepted));
