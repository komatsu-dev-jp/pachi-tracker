...
test("店舗台番号候補とグラフ番号が矛盾する行を最高出玉だけで確定しない", () => {
  const result = matchSiteSevenGraphPanels([{
    panelId: "g-480",
    observedNumCandidate: "480",
    machineNumberAccepted: true,
    maxPayout: 1000,
    maxPayoutAccepted: true,
  }], [{
  ...
  assert.deepEqual(result.unmatchedGraphs.map((panel) => panel.panelId), ["g-480"]);
  assert.deepEqual(result.unmatchedRows.map((row) => row.rowId), ["t-review"]);
  assert.ok(result.reviewReasons.some((reason) => (
    reason.code === "number-candidate-conflict"
      && reason.graphIds.includes("g-480")
      && reason.rowIds.includes("t-review")
  )));
});
...
test("資料間で競合したPDF・写真行はacceptedでも共同照合の固定点にしない", () => {
  const graphPanels = [{
    panelId: "g-479",
    num: 479,
    machineNumberAccepted: true,
  ...
  assert.equal(result.matches.length, 0);
  assert.deepEqual(result.unmatchedGraphs.map((panel) => panel.panelId), ["g-479"]);
  assert.deepEqual(result.unmatchedRows.map((row) => row.rowId), ["pdf-479"]);
});

test("通常回転数だけの要確認ならaccepted済み台番号と最高出玉は照合に使う", () => {
  const result = matchSiteSevenGraphPanels([{
    panelId: "g-479",
    num: 479,
    machineNumberAccepted: true,
    ...
    reviewRequired: true,
    reviewConfirmed: false,
    fieldReviewRequired: { normalSpins: true },
  }]);

  assert.equal(result.matches.length, 1);
  assert.equal(result.matches[0].resolvedNum, "479");
});
