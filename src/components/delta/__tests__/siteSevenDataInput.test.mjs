...
test("setSiteSevenRowsReviewConfirmation: 指定した要確認台だけを一括確認し、解除もできる", () => {
  const rows = [
    {
      num: "479",
      reviewRequired: true,
      reviewConfirmed: false,
      editedFields: ["normalSpins"],
      fieldAccepted: { normalSpins: false },
    },
    { num: "480", reviewRequired: true, reviewConfirmed: false },
    { num: "481", reviewRequired: false, reviewConfirmed: false },
  ];

  const confirmed = setSiteSevenRowsReviewConfirmation(rows, true, [0, 2]);
  assert.equal(confirmed[0].reviewConfirmed, true);
  assert.equal(confirmed[0].fieldAccepted.normalSpins, true);
  assert.strictEqual(confirmed[1], rows[1], "対象外の要確認台は変更しない");
  assert.strictEqual(confirmed[2], rows[2], "確認不要の台は対象指定されても変更しない");

  const cleared = setSiteSevenRowsReviewConfirmation(confirmed, false, [0]);
  assert.equal(cleared[0].reviewConfirmed, false);
  assert.strictEqual(setSiteSevenRowsReviewConfirmation(rows, true), rows, "対象未指定では一括確定しない");
});

test("applySiteSevenMachineNameToRows: 機種名を全台または指定台へ手動補正として反映する", () => {
  const rows = [
    {
      num: "479",
      machineName: "旧機種A",
    ...
    { num: "480", machineName: "旧機種B" },
  ];

  const all = applySiteSevenMachineNameToRows(rows, "  P新機種  ");
  assert.deepEqual(all.map((row) => row.machineName), ["P新機種", "P新機種"]);
  assert.ok(all.every((row) => row.machineNameSource === "manual"));
  assert.equal(all[0].storeLayoutRelation.status, "manual-override");
  assert.equal(all[0].storeLayoutRelation.machineNameApplied, false);

  const selected = applySiteSevenMachineNameToRows(rows, "P別機種", [1]);
  assert.strictEqual(selected[0], rows[0]);
  assert.equal(selected[1].machineName, "P別機種");
  assert.strictEqual(applySiteSevenMachineNameToRows(rows, "   "), rows, "空文字では既存名を消さない");
});
