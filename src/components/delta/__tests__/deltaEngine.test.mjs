...
test("途中の境界接触は確定し、短線と境界で切れた終点は制約付きreviewにする", () => {
  ...
  assert.equal(results[1].status, "ok", "途中の境界接触だけでは終点値を要確認にしない");
  assert.ok(results[1].reasonCodes.includes("historical-boundary-contact"));
  assert.ok(!results[1].reasonCodes.includes("clipped-series"));
  assert.equal(results[1].valueConstraint, null);
  ...
  assert.ok(results[2].reasonCodes.includes("endpoint-clipped-bottom"));
  assert.equal(results[2].valueConstraint, null);
  assert.deepEqual(results[2].boundaryObservation, {
  ...
  assert.ok(results[3].reasonCodes.includes("clipped-series"));
  assert.ok(results[3].reasonCodes.includes("endpoint-clipped-top"));
  assert.equal(results[3].valueConstraint, null);
  assert.deepEqual(results[3].boundaryObservation, {
    kind: "censored-top",
    boundary: "top",
    value: 30000,
    exact: false,
  });
  assert.ok(results[3].val >= 29500);
...
test("途中で上端へ触れ終端が近いだけならhard下限にしない", () => {
  const image = createRgba(260, 120);
  drawPanel(image, {
    x: 10,
    y: 10,
    zeroY: 50,
    spacing: 12,
    series: [[6, 50], [45, 14], [80, 15]],
  });

  const { results } = runAnalysis(image.data, image.width, image.height);
  assert.equal(results.length, 1);
  assert.equal(results[0].status, "ok");
  assert.ok(results[0].reasonCodes.includes("historical-boundary-contact"));
  assert.ok(!results[0].reasonCodes.includes("endpoint-clipped-top"));
  assert.equal(results[0].valueConstraint, null);
});

test("下端へ向かう終点が近いだけなら接触値を作らず要確認にする", () => {
  const image = createRgba(260, 120);
  drawPanel(image, {
    x: 10,
    y: 10,
    zeroY: 50,
    spacing: 12,
    series: [[6, 50], [70, 80], [80, 85]],
  });

  const { results } = runAnalysis(image.data, image.width, image.height);
  assert.equal(results.length, 1);
  assert.equal(results[0].status, "review");
  assert.ok(results[0].reasonCodes.includes("endpoint-near-bottom"));
  assert.ok(results[0].reasonCodes.includes("boundary-uncertain"));
  assert.equal(results[0].boundaryObservation, null);
  assert.equal(results[0].valueConstraint, null);
});
