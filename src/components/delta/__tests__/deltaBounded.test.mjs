...
test("境界到達記録に一点値またはランクが混ざった行は有効扱いしない", () => {
  ...
  assert.equal(isBoundedDeltaRow({ ...bounded, val: "" }), false);
  assert.equal(isBoundedDeltaRow({ ...bounded, val: "abc" }), false);
  assert.equal(isBoundedDeltaRow({ ...bounded, val: undefined }), false);
