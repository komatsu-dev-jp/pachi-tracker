...
test("実画像52台のうち元の要確認5台を、終点と過去経路を区別して判定する", async () => {
  ...
  const expected = new Map([
    ["499", { val: 28000, status: "review", reasonCodes: ["endpoint-clipped-top", "clipped-series"] }],
    ["503", { val: -2000, status: "review", reasonCodes: ["short-series"] }],
    ["508", { val: -3500, status: "review", reasonCodes: ["short-series"] }],
    ["548", { val: 28500, status: "review", reasonCodes: ["endpoint-clipped-top", "clipped-series"] }],
    ["574", {
      val: 29500,
      status: "review",
      reasonCodes: ["endpoint-near-top", "boundary-uncertain"],
    }],
  ]);
  ...
  assert.equal(endpointClipped.valueConstraint, null, "台574: 近接だけで30,000玉以上を断定しない");
  assert.equal(endpointClipped.boundaryObservation, null, "台574: 近接だけを境界接触として記録しない");
  assert.deepEqual(byNumber.get("548").boundaryObservation, {
    kind: "censored-top",
    boundary: "top",
    value: 30000,
    exact: false,
  }, "台548: 線の中心値を30,000玉以上と断定せず接触事実を記録する");
  assert.deepEqual(byNumber.get("499").boundaryObservation, {
    kind: "censored-top",
    boundary: "top",
    value: 29000,
    exact: false,
  }, "台499: 上限で消えた終点を途中接触として誤確定しない");
  ...
  assert.deepEqual(counts, { ok: 47, review: 5 });
});
