...
### 4-4. ヘッダー圧縮（`.rec-ios-header.is-condensed`）

ヘッダーは固定（`flexShrink: 0`）のため、展開時は 259px を占めます。
データを読む間はこれが邪魔になるので、**スクロールすると畳みます**（iOS のラージタイトル相当）。

| 状態 | 高さ | 表示されるもの |
|---|---|---|
| 展開（先頭） | 259px | 日付＋今日の実戦＋通知・設定／店舗名＋投資ペース／機種カード／サブタブ |
| 圧縮（スクロール中） | 136px | 機種カード（行高を詰める）／サブタブ |

畳むのは**装飾と静的情報だけ**です。実戦中に変化する値（機種カードの現金投資・回転数）と
サブタブは常に残します。通知ベル・設定ボタンは先頭までスクロールすれば戻ります。

- 判定は `RotTab.jsx` の `headerCondensed`。ルート要素で `scroll` をキャプチャして拾います
- サブタブ本体のスクロール領域には **`rec-ios-scroll` クラスが必須**です。
  これが無い要素のスクロールは無視します（モーダル内リストや候補ドロップダウンで
  ヘッダーが畳まれるのを防ぐため）。新しいスクロール領域を足すときは必ず付けること
- しきい値は畳み 28px / 戻し 6px のヒステリシス。境界でのちらつきを防ぎます
- サブタブ切替時は圧縮を解除します（新しいスクロール領域は先頭から始まるため）
- `prefers-reduced-motion: reduce` ではトランジションを切ります
