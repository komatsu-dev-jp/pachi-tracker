...
台移動の入口と確定には連打防止を追加し、確認中・移動中の表示と、保存開始後の例外を再試行させない fail-closed（安全のため操作を止める仕組み）を実装しました。保護対象、保存処理、計算式は不変です。ブランチは `codex/transfer-button-guard-20260803`、通常のタップ数は2回で、テスト・lint・build・UI確認を実施対象とし、未確認事項は実機iOS Safariです。

最終更新: 2026-08-13（**ホーム画面と台選び画面をモック準拠の iOS デザインへ刷新**（ブランチ `claude/pachi-ios-home-machine-select-rgwioj`）。**経緯**: ユーザーから「ホーム画面と台選びをモックの Design 案を参考に忠実に再現。数式は変更しないように」と依頼。出典モックは `docs/design-review/pachi-ios-redesign.html`（記録モードの iOS 刷新 PR #465・#466 と同じモック）。**新規ファイル**: `docs/home-select-ios-design.md`（ホーム・台選びの iOS デザイン仕様書）。**変更ファイル**: `src/components/home/HomeDashboard.jsx` / `.css`、`src/components/strategy/StrategyMapDashboard.jsx` / `.css`、`src/index.css`（`.strategy-map, .yutime-sheet` のトークンブロックのみ）、`CLAUDE.md`（仕様書への誘導5行）。**①重要な前提**: フッターの「台選び」タブが開くのは `SelectDashboard` ではなく **`StrategyMapDashboard`（戦略マップ画面）**（`App.jsx:1877` の `currentMode === "select" || "strategy"` 分岐）。`SelectDashboard.jsx` はどのタブからも到達しないため今回の対象外とし無変更。**②ホーム画面**: ヘッダーを「ブランド（11px muted）＋日付（22px）＋44px の丸型通知ベル」＝モックの `.app-bar` へ再構成し、その下に「店舗名（18px）＋今月の記録件数／右に実戦中・実戦前のピル」＝モックの `.page-intro` を新設（旧 `.home-header__top` / `h1` / `.home-header__status` の3ブロックを置換。表示していた情報は全て維持）。各カードの見出し（`SectionTitle`）を**カードの中から外へ**移し、モックの `.group-label`（11px・muted・`letter-spacing .06em`・右は systemBlue）に統一。「次にやること」の遷移ボタンを行内の小ボタンからモックの `.primary-action`（幅100%・50px・角丸14px・systemBlue 塗り）に変更し、「差玉を解析」も同寸法へ。直近の記録のアイコンをオレンジのグラデーションから青の淡い塗り（モックの `.machine-icon`、40px・角丸12px）へ。カードは角丸15/14px→**16px**・枠は hairline・影とグラデーションを全廃。**③台選び画面**: ヘッダーを「丸型44pxの戻る＋解析日（小）＋台選び（22px）＋丸型44pxのヘルプ」＝`.app-bar` と、「機種名＋選択台・表示範囲／右に候補台数ピル」＝`.page-intro` に再構成。全台／良台候補のタブを、シアンのグラデーション＋発光からモックの `.segmented`（溝＋つまみ）へ。KPI 4枚のカードを**1枚のカードを区切り線で2×2に割る** `.metric-grid` へ。実戦開始ボタンをグラデーション＋影から `.primary-action` へ。`Section`（画面共通の見出し）をグループ見出しへ変更し、`accent` は 7px のドットで併記。カード角丸 `RADIUS` 24→**16**。**④配色**: `.strategy-map, .yutime-sheet` の `--sm-*` を iOS システムカラーへ差し替え（`--sm-cyan` の実体が systemBlue になった。**トークン名から色を推測しないこと**）。`--sm-faint` / `--sm-panel-3` / `--sm-purple` を新設。ホームは `HomeDashboard.css` 冒頭に `--ho-*`（iOS）を新設し、既存の `--home-blue/cyan/green/yellow/red` はそのエイリアスに変更（既存クラスを触らずに配色だけ追従）。汎用トークン（`--bg`/`--surface`/`--text` 等）も `.home-dashboard` 内で上書きし、Recharts のグラフ色も追従。**両ファイルからハードコードの色（旧スレート/ネイビー/シアン系 約60箇所）を全て排除**し、残るリテラルは `#fff` / `#000` / 影の黒のみ。発光（`box-shadow: 0 0 Npx <色>` / `text-shadow`）は全廃し、選択状態は `box-shadow: 0 0 0 2px <アクセント>` に統一。**⑤副次的な影響**: `--sm-*` を共有する遊タイム計算シート（`YutimeCalculatorSheet`、記録モードからも開く）が配色だけ iOS 化。記録モード（`.rec-ios`）と同じパレットのため整合は取れている（構造は無変更）。**保護**: `src/logic.js` 変更なし（`git diff --quiet` 確認・`node src/__tests__/protected-fns.mjs` の出力が `baseline.json` と**完全一致**）。`evDecision.js` / `baseline.json` も差分なし。計算式・集計セレクタ・保存データ構造は無変更。既存の表示項目は1つも削除していない。**操作ステップ**: 増減なし（タップ数不変。見出しの移動と配色・寸法の変更のみ）。**package.json 変更なし**。**検証**: `npm run lint`=エラー0・警告0、`npm run build`=成功（既存の chunk>500kB 警告のみ）、`npm test`=705 pass / 0 fail / 4 skip。Playwright+Chromium 実描画（393×852 / 320×852、`hasTouch`/`isMobile`）でダーク・ライト両テーマ、ホーム（記録あり／記録ゼロ）と台選び（島・差玉あり／データなし）を確認し、全ケースで**横はみ出し0件・コンソールエラー0件**。記録・分析・設定タブに回帰がないことも実描画で確認。**未対応・残課題**: (a) 台選びの**カード内タイトル**（「島活動シグナル履歴」「翌日予測の答え合わせ」等）はカードの中に残置（見出しを4段外へ出すと親子関係が読めなくなるため。仕様書 §8 に理由を明記）、(b) ホームの「月間目標から自動逆算」パネルの破線枠は「未設定」を示す既存表現として残置、(c) 実機（iOS Safari）での最終確認は未実施。**前回までの経緯は下記**。）

最終更新: 2026-07-26（**分析ページ iOS 刷新の総仕上げ：デザイン仕様書を新設＋取りこぼしの iOS 化**（ブランチ `claude/analytics-ios-redesign-2tmqil`、同日の iOS 刷新2エントリの締め）。**経緯**: ユーザーから「Codex が後から見ても分かるようにしてほしい」との指示。**新規ファイル**: `docs/analysis-ios-design.md`（分析ページ iOS デザイン仕様書）。**仕様書の内容**: ①スコープ表（どのファイルが対象で、`Tabs.jsx` の CAL カレンダー＝`.cal-terminal` は対象外であること）、②デザイン方針（インセットグルーブド／見出しはカードの外／`font-black`・`font-mono` を使わない／色はトークン経由／タップ44px）と**意図的な例外**（`ShareCard` は共有画像のモックなのでポスター的な太字のまま）、③**デザイントークン早見表**（ダーク／ライトの実値と用途。トークン名は金融端末風時代のままなので名前から色を推測しないこと、`--at-on-gold` が要る理由も明記）、④コンポーネント規約（`card` / `GroupLabel` / `RoundButton` / ナビゲーションバー / セグメンテッドコントロール / リスト行 / 主ボタン / アクションシートのコピペ用スニペット）、⑤数値表示ルール（`tabular-nums` 必須・`signed`/`fmt`/`moneyClass`・**万表記に戻さない**経緯）、⑥収支カレンダー仕様（セル構成図・日付バッジの状態優先順・`HEAT_STRONG_YEN` による2段階ヒートの対応表・`cellAmountSize` の注意・週別収支を表の下に置いた理由・「今日」ボタン）、⑦**画面遷移スタックの表現方法**（react-router 非導入のため state の早期 return で積む。判定順＝スタックの深さ。サブ画面の state は親に持たせる理由）、⑧触ってはいけない範囲（`logic.js` / `evDecision.js` / 集計セレクタの計算式 / 保存データ構造 / `baseline.json`）と「表示専用の純関数ならオプション追加可」の線引き、⑨検証手順（lint/build/`git diff --quiet src/logic.js`/`protected-fns.mjs`＋Playwright の確認マトリクスと横はみ出し検出スニペット）、⑩**落とし穴 5 件**（SVG の presentation attribute は `var()` を解決しない／`shadow-*` 二重指定は勝敗不定で枠は `outline` を使う／`Bar` に既定 `fill` がないと凡例が黒くなる／`new Date(y,m-1,31)` の繰り上がり／`touch-pan-y` と `overflow-x:clip` を外すと iOS でブレる）。**CLAUDE.md**: 「分析ページを触るとき（必読）」の 5 行を追加し、この仕様書へ誘導（着手前に必ず読む旨）。**取りこぼしの iOS 化**: (a) 共有モーダルの操作ボタン2つ（明色カードの外＝ダークの被せ面）を 50px の iOS ボタンへ、(b) 記録編集シート（分析→記録を編集/追加）のヘッダーを丸型戻る＋17px タイトル＋日計ピルの iOS ナビゲーションバーへ。**保護**: `src/logic.js` 変更なし（`git diff --quiet` 確認・`protected-fns.mjs` が `baseline.json` と完全一致）。計算式・集計セレクタ・保存データ構造は無変更。**検証**: `npm run lint`=エラー0・警告0、`npm run build`=成功、テスト 55 ファイル＋`ball-consumption` 21 件すべて失敗0。Playwright 実描画（393/320px × ダーク/ライト）で全画面を再確認し、**横はみ出し 0 件・コンソールエラー 0 件**。**前回までの経緯は下記**。）

最終更新: 2026-07-26（**店舗トップ3「すべて見る」を実装＋分析ページ配下の全画面を iOS 風で統一**（ブランチ `claude/analytics-ios-redesign-2tmqil`、同日の iOS 刷新エントリの続き）。**経緯**: 前エントリで残課題としていた「『すべて見る』の遷移先が未実装」をユーザー指示で実装。あわせて未刷新だった詳細分析配下（AnalyzerView / 店舗詳細 / 機種詳細）のデザインを iOS 風に統一。**`Tabs.jsx` の CAL カレンダー（証券端末風）はユーザー判断により対象外**（`.cal-terminal` トークンは無変更のまま残す）。**変更ファイル**: `src/components/analysis/AnalysisDashboard.jsx`、`src/components/analysis/AnalyzerView.jsx`、`src/components/analysis/AnalyticsCharts.jsx`、`src/index.css`（トークン1つ追加のみ）。**①「すべて見る」＝店舗一覧画面（新規 `StoreListScreen`）**: 詳細分析の店舗トップ3見出し横のラベル（従来は遷移先なしの飾り）を `すべて見る（n）›` のボタンにし、全店舗の一覧サブ画面へ遷移させた。画面構成は iOS のプッシュ遷移相当で、丸型の戻る＋タイトル＋店舗数、合計収支/期待値/稼働の3列サマリー、**並び替えセグメンテッドコントロール（実収支順／期待値順／稼働日数順）**、全店舗のリスト（順位バッジ・店舗名・期待値/稼働日数/平均回転率・実収支・シェブロン）。行タップで既存の `StoreDetailScreen` へ進み、戻ると一覧に復帰する（`storeListOpen` と `storeDetailName` の2状態でスタックを表現）。**並び替えの選択は親（`storeListSort`）で保持**し、店舗詳細へ往復しても選択が消えない。**集計は既存の `buildStoreRanking` に `{ sortBy, limit }` オプションを追加しただけ**で、合算式（`getEvAmount` / `getActualPL`）そのものは無変更。`limit: 0` で全件を返す。既定の呼び出し（トップ3用）は従来どおり `sortBy:"ev", limit:5` のため、既存表示は不変。**②店舗詳細 `StoreDetailScreen`**: 英字ラベル `STORE REPORT` と `全期間` ピルを廃し、iOS のナビゲーションバー（丸型戻る＋店舗名）＋ヒーローカード（収支・平均回転率）へ再構成。1位バッジはヘッダー右の金色ピルに集約。実施履歴は iOS のグループ化リスト（60px 行・区切り線）へ。**③機種詳細 `MachineDetailScreen`**: 同じく `MACHINE REPORT` を廃してナビゲーションバー化、実戦/勝率/時給の3ピルをヒーローカード内の3列へ格納。実戦履歴に**列見出し行（日付・店舗／収支／期待値／累計）を新設**（3つの数値列が何なのか分からなかったため）。**台番号は店舗名の隣から下段のピルへ移動**し、店舗名が省略されないようにした（台番号順ソート時の視認性も向上）。並び替えも iOS セグメンテッドコントロールへ。**④AnalyzerView / AnalyticsCharts**: 内部タブ（期待値分析/店舗分析/機種分析）と機種の並び替えを iOS セグメンテッドコントロール（溝＋つまみ）へ。`MetricCell` を iOS のスタットタイル（塗り面・ラベル12px・値19px semibold）へ。1位機種カード `FeaturedMachineCard` の金グラデーション背景をやめ、他カードと同じ面＋金の outline に統一。チャート色も iOS システムカラーへ。**⑤トークン追加**: `--at-on-gold`（ダーク `#3d2f00` / ライト `#ffffff`）。ライトテーマの金 `#B25000` は濃色のため、順位バッジの文字が読めなくなるのを防ぐ。**保護**: `src/logic.js` 変更なし（`git diff --quiet` で確認・`protected-fns.mjs` が `baseline.json` と完全一致）。`evDecision.js` / 計算式 / 集計セレクタ（`analysisSelectors.js` / `analyticsViewSelectors.js` / `analyzerSelectors.js`）/ 保存データ構造は無変更。既存の表示項目は削除していない。**操作ステップ**: 「すべて見る」は 1 タップで全店舗一覧へ（従来は到達手段なし）。既存フローのタップ数は不変。**package.json 変更なし**。**検証**: `npm run lint`=エラー0・警告0、`npm run build`=成功、`protected-fns.mjs` が `baseline.json` と完全一致、テスト 56 ファイル失敗 0。Playwright+Chromium 実描画（393×852 / 320×852）でダーク・ライト両テーマ、月別/年別/通算/詳細分析/月次詳細/表示切替シート/絞り込み/記録追加シート/空状態/7桁金額、および**「すべて見る」→一覧→並び替え→店舗詳細→戻る（並び替え保持）**、機種分析→機種詳細（列見出し・台番号ピル）、共有カードを確認。全ケースで**横はみ出し 0 件・コンソールエラー 0 件**。**未対応**: (a) `Tabs.jsx` の CAL カレンダーは対象外（ユーザー判断）、(b) 店舗詳細の「平均回転率」は `storeAnalysis` が 0 を返す記録では `0.0回/k` と出る（既存セレクタ由来・今回未変更）、(c) 実機（iOS Safari）確認は未実施。**前回までの経緯は下記**。）

最終更新: 2026-07-26（**分析ページのデザインを iOS ネイティブ風へ全面刷新＋収支カレンダーの可読性改善**（ブランチ `claude/analytics-ios-redesign-2tmqil`）。**経緯**: ユーザーから「分析ページもデザインを iOS 向けに刷新。カレンダーを見やすくして収支管理をしやすく。収支管理アプリがメイン」と依頼。**変更ファイル**: `src/index.css`（`.analytics-terminal` とライト上書きのデザイントークンのみ）、`src/components/analysis/AnalysisDashboard.jsx`（UIのみ）。**①デザイントークンの iOS 化**: 金融端末風（ネイビー/ネオンシアン/グラデーションカード/強い影）から iOS システムカラーへ差し替え。ダーク=背景 `#000` / カード `#1C1C1E`（secondarySystemGroupedBackground）/ アクセント `#0A84FF`(systemBlue) / プラス `#30D158` / マイナス `#FF453A`、ライト=背景 `#F2F2F7` / カード `#FFF` / `#007AFF` / `#248A3D` / `#D70015`。カード影は実質廃止、区切り線は iOS の separator 相当（`rgba(84,84,88,…)` / `rgba(60,60,67,…)`）へ。**トークン名は据え置き**のため、同じ `--at-*` を参照する `AnalyzerView` / `AnalyticsCharts` / `Tabs.jsx` の編集シート（focusMode。`--at-*` の使用箇所は focusMode のみで、これは `.analytics-terminal` 配下で描画される）も自動追従し、分析ページ全体で配色が一致する。新規トークンは `--at-heat-p2` / `--at-heat-m2`（カレンダーの濃いヒート塗り）のみ。**②収支カレンダーの可読性**: セル枠線を廃し「日付＝丸バッジ（上）/ 金額＝下段」の iOS カレンダー構成へ。空白セルは描画しない。ヒート塗りを絶対値 3万円（表示専用定数 `HEAT_STRONG_YEN`）で 2 段階にし大勝ち・大負けを判別可能に。カード余白 `p-2.5`→`px-2 py-3`・セル間 `gap-1`→`gap-[3px]` でセル幅を約 47px→約 49px に拡大し、`cellAmountSize` の各段階を 1〜2px 引き上げ（5桁以下 12/13px 〜 10桁以上 7/8px。**金額は従来どおり実額表示のまま**、万表記へは戻していない）。セルは `aspect-square`→`aspect-[1/1.12]` で高さ約 54px（タップ領域 44px 基準を満たす）。**③新規追加（収支管理の実用性）**: (a) **今日マーカー**（表示中が今月のときだけ該当日を薄い丸＋青文字。従来は今日が全く分からなかった）、(b) **「今日」ボタン**（カレンダー見出し右。何ヶ月さかのぼっても1タップで今月＋本日へ復帰）、(c) **週別収支の 1 行**（カレンダー下。第n週ごとの実収支合計。8列目を足すと金額が読めなくなるため表の下に配置）、(d) **ヒーローに 投資 / 回収 / 稼働日数 の 3 列を追加**（`summary.totalInvest` / `totalRecovery` / `days` をそのまま表示。従来の 月間収支・期待値・差・勝率リングは維持）。**④その他 UI**: ヘッダーを中央絶対配置から「左＝‹ 年月⌄ › / 右＝月次詳細・絞り込み」の 2 グループへ再構成（丸型ボタン 44px）、表示切替メニューを iOS アクションシート（56px 行＋チェックマーク＋独立したキャンセルボタン）へ、絞り込みを iOS セグメンテッドコントロール＋フォーム行へ、日別詳細を「日付見出し＋2×2 の指標カード（数値 11px→19px）＋青の塗りつぶしボタン（50px）」へ、SNS共有CTAを iOS リスト行へ。フォントは全体的に `font-black`(900)/`font-mono` を `font-bold`/`font-semibold`＋`tabular-nums` へ置換（SF 系の見た目に寄せる）。Recharts の配色も iOS システムカラーへ（`#30D158`/`#0A84FF`/`#FF9F0A`/`#8E8E93`。**SVG の presentation attribute は `var()` を解決しないため軸・グリッドの stroke はリテラル値**、tooltip の `contentStyle` は style オブジェクトなのでトークン参照）。**⑤ついでに直した表示バグ**: (a) 選択日の初期値が **14 固定**だったのを**今日**に変更、(b) 31日を選んだまま 30 日以下の月へ送ると `new Date(y, m-1, 31)` が翌月へ繰り上がり曜日ラベル・日別詳細がずれていたため、`activeDay = min(selectedDay, その月の日数)` で丸めてから使用（state は不変。UI 側の導出のみ）、(c) 月次詳細グラフの凡例「日別収支」のスウォッチが黒だったのを `Bar` の既定 `fill` 指定で修正。**保護**: `src/logic.js` は変更なし（`git diff --quiet src/logic.js` で確認。`node src/__tests__/protected-fns.mjs` の出力が `baseline.json` と完全一致）。`evDecision.js` / 計算式 / 集計セレクタ（`analysisSelectors.js` 等）/ 保存データ構造 / `rotRows` は無変更。既存の表示項目は削除していない（機能未実装の「すべて見る」ラベルも見出し横に残置。**要判断: 遷移先が無いままなので撤去も可**）。**操作ステップ**: 増加なし。「今日」ボタン（最大数タップの月送りが 1 タップに）と今日マーカーは純増の導線。**package.json 変更なし**（新規ライブラリなし）。**検証**: `npm run lint`=エラー0・警告0、`npm run build`=成功（既存の chunk>500kB 警告のみ）、`protected-fns.mjs` が `baseline.json` と完全一致、ユニットテスト全 51 ファイル PASS（fail 0）。Playwright+Chromium 実描画（393×852 / 320×852、`hasTouch`/`isMobile`）でダーク・ライト両テーマ、月別 / 年別 / 通算 / 詳細分析 / 月次詳細 / 表示切替シート / 絞り込み / 記録追加シート / 記録ゼロの空状態 / 7桁金額（-1,250,000 等）を確認。全ケースで**横はみ出し 0 件・コンソールエラー 0 件**。**未対応・残課題**: (a) `Tabs.jsx` の CAL カレンダー（証券端末風）は今回の対象外で旧デザインのまま、(b) 日別詳細の稼働時間は従来どおり dayMap 未連携時 `—`、(c) 実機（iOS Safari）での最終確認は未実施（ヘッドレス Chromium での検証）。**前回までの経緯は下記**。）

最終更新: 2026-07-15（**アプリ全体バグ調査＋App Store販売前提の改善（バグ3件修正・引き継ぎレポート作成）**（ブランチ `claude/app-bugs-app-store-q6fjeb`、コミット `1aaec53`・`1f6ca39`、実装・調査: Fable）。**経緯**: ユーザーから「アプリ全体でまだバグはあるか？Apple Store販売を前提に調査改善」と依頼。全サポートモジュール（persistence/snapshot/notifications/history/settingsUtils/csv/haptics/ballConsumption/evidence/yutimeCalculator/hunterRank/analysisSelectors/scoutSelectors等）と App.jsx を精読、Tabs.jsx はパターン横断検索で監査した。**修正①（最重要・UTC日付バグ）**: `new Date().toISOString().slice(0,10)` が UTC 基準のため、日本時間 0:00〜9:00 に「今日」が前日扱いになるバグを全16箇所で修正。`src/constants.js` に `localDateStr()`（端末ローカル YYYY-MM-DD）を追加し、App.jsx（アーカイブ記録日・セッション開始日/持越し検知・貯玉日次リセット・連続稼働日数・貯玉履歴、6箇所）、Tabs.jsx（セッション開始日・貯玉履歴/フォーム日付・バックアップ/CSVファイル名、8箇所）、HomeDashboard.jsx（「今日/前回」判定）、DeltaAnalyzer.jsx（todayStr）、deltaSelectors.js（makeScan既定日・pruneScans保持期限。nodeテスト独立性のためモジュール内ヘルパー `toLocalDay` を定義）を置換。**修正②**: HomeDashboard「直近の記録」の回転率が `netRot/1000`（総回転数÷1000）という無関係な値を表示していたバグを修正。`stats.effectiveStart1K`（無ければ `start1K`）を表示し、値が無い場合はダミー「19.2 /k」でなく「-- /k」を表示。**修正③**: `downloadCSV` の `URL.revokeObjectURL` 即時実行（iOS Safariで保存中断の恐れ）を body append + 1秒遅延 revoke に変更（backupAllData と同一方式に統一）。**報告のみ（未修正・ユーザー判断待ち）**: (1) `logic.js:168` の `mochiCostPerK = 1000×exRate/rentBalls` は非等価店で交換価値と逆方向にスケールする疑義あり（数学的には `1000×rentBalls/exRate` が正しい可能性。App.jsx の chodamaYen 2箇所も同規約で整合させてあるため、直す場合は両方同時に境界値検証付きで。logic.js は変更禁止のため報告のみ）。(2) HomeDashboard.jsx にダミーデータ残存＝DEMO定数（+48,500円等）・**MaruhanMark（実在企業マルハンのロゴ風マーク+店名）**・NextActionCard/JudgmentCard（8件/2件固定）/ActiveStoreCard（分析度78%等）のハードコード表示。App Store ガイドライン 5.2.1（商標）/2.3.1 のリジェクトリスクがあり実データ連携 or 非表示への差し替えを最優先で推奨（既存UI勝手削除禁止の規定に従い提案に留めた）。(3) App Store 準備事項: Capacitor 等のネイティブラッパー化が必要（その際 vite.config.js の `base: '/pachi-tracker/'` は `./` へ）、レーティング17+必須、AI読み取りのAPIキー端末保存・直接送信に関するプライバシーポリシー整備、viewport `user-scalable=no` のアクセシビリティ、Vibration API は iOS 非対応（ラッパー化時に Capacitor Haptics へ）。**引き継ぎドキュメント**: 詳細と Codex への作業依頼範囲（着手可＝ホーム画面の見た目差し替え／禁止＝logic.js・evDecision.js・保存データ構造）を `docs/report-2026-07-15-bug-audit-appstore.md` に整理済み。**保護**: `src/logic.js`・`src/components/decision/evDecision.js`・計算式・保存データ構造は無変更。操作ステップ数の変化なし。package.json 変更なし。**検証**: `npm run lint`=エラー0、`npm run build`=成功、`protected-fns.mjs` 出力が `baseline.json` と完全一致、全ユニットテスト194件PASS（コンポーネント103 + evidence/settings 70 + ball-consumption 21）。**前回までの経緯は下記**。）

最終更新: 2026-07-11（**分析タブ日別ヒートマップを万表記から実額表示へ変更**（ブランチ `claude/calendar-number-overflow-ibwad2`、実装: Sonnet / 検収・指揮: Fable）。**経緯**: 直前のエントリで1万円以上を万表記（`-1.7万`）に短縮したが、ユーザーから参考アプリ（家計簿カレンダー）同様に実額のまま表示したいと要望があり方針変更。**対応**（`src/components/analysis/AnalysisDashboard.jsx` のみ）: `formatCellAmount`（万表記変換）を削除し、桁数でフォントサイズクラスを返す `cellAmountSize`（7文字以下=11px / 8文字=10px / 9文字以上=9px）に置換。金額表示は `signed(amount)`（実額・カンマ付き）へ復帰。幅確保のため CalendarPanel のカード余白 p-3.5→p-2（見出し行に px-1.5 追加で緩衝）、曜日見出し・セル grid の gap-1→gap-0.5、CalendarCell の px-1→px-0（日付 span に pl-1 追加）、金額 span の font-mono を削除（通常フォント+tabular-nums の方が横幅が狭い）、tracking -.04em→-.03em。金額に使える幅は約33px→約44pxに拡大。**副次効果**: セルが約42.6px→約46px角となりタップ領域44px基準を満たすようになった。**保護**: `src/logic.js`・`src/evDecision.js`・計算式・保存データ構造は無変更。ヒート塗り・選択枠・日付色・onSelect・日別詳細パネルも無変更。操作ステップ数の変化なし。**検証**: `npm run lint`=0 errors（既存警告1件のみ）、`npm run build`=成功。Playwright+Chromium（390×844、vite preview）で `pt_archives` 既存構造どおりのテストデータ（+84,226 / -104,500 / -19,668 / +318）を注入し、6桁マイナスを含む全額がセル枠内に収まることを実描画スクリーンショットで確認済み。**メモ**: `useLS` の実体は IndexedDB(Dexie) バックで、初回起動時に localStorage の `pt_*` キーを自動移行する仕組み（`persistence.js` の `migrateFromLocalStorage`）があることを検証中に再確認。**前回までの経緯は下記**。）

最終更新: 2026-07-11（**分析タブ日別ヒートマップのセル内金額はみ出しを修正**（ブランチ `claude/calendar-number-overflow-ibwad2`、実装: Sonnet / 検収・指揮: Fable）。**問題**: `CalendarCell` が日別収支を実額（例 `-17,168`）のまま等幅9.5pxで描画しており、iPhone 16 Pro相当のセル内寸（約34px）に対して5桁+符号+カンマの7文字（約38px）が既にはみ出し、10万円以上では `overflow-hidden` で確実に見切れていた。**対応**: `src/components/analysis/AnalysisDashboard.jsx` に表示専用の純粋関数 `formatCellAmount` を追加（1万円未満=実額 `signed`、1万〜100万未満=万表記・小数1桁・四捨五入 例 `-1.7万`/`+10.5万`、100万以上=万表記・整数 例 `+123万`。分岐は絶対値ベースで `-0.0万` 事故を回避）。セル金額のフォントを9.5px→9px、ボタン水平パディングを px-1.5→px-1 に調整し最長ケース `+10.5万` が枠内に収まることを確認。**設計判断**: 既存コメントの「実額表示」は英語 `k` 表記回避が意図だったため、日本語UIルールと整合する「万」表記へ変更。正確な金額はセルタップ後の日別詳細パネルに従来どおり表示され情報欠落なし。**スコープ外（意図的に未対応）**: `Tabs.jsx` 側のCALカレンダー（証券端末風）は同種の短縮が未適用のまま。必要ならユーザー確認のうえ別タスクで対応。**保護**: `src/logic.js`・`src/evDecision.js`・計算式・保存データ構造は無変更。変更は表示フォーマットのみ。操作ステップ数の変化なし。**検証**: `npm run lint`=0 errors（既存警告1件のみ・無関連）、`npm run build`=成功、境界値（0/±9,999/±10,000/±17,168/±104,500/±999,999/±1,000,000超）を node で確認済み。**前回までの経緯は下記**。）

最終更新: 2026-07-11（**店舗詳細画面（概要/設定タブ）を実店舗データに接続**（ブランチ `claude/new-session-48im8n`）。**目的**: 遷移導線（前回エントリ参照）はどのボタンを押しても常に同じダミー「マルハン空港通店」を表示していたため、`storeId` から実店舗（`S.stores`＝`pt_stores`）を解決して概要/設定タブに反映（着手前にユーザーへ次の優先タスクを確認済み：「storeIdに基づく実店舗解決」を選択）。**新規ファイル**: `src/components/store/storeDetailSelectors.js`（`resolveStoreDetail(stores, storeId, opts)` 純粋関数）。**変更ファイル**: `src/pages/StoreDetail.jsx`（`MOCK_STORE_DETAIL` 直接参照をやめ `resolveStoreDetail` の結果を使用。`S` prop を新規受領）、`src/App.jsx`（`<StoreDetail>` に `S={S}` を追加で渡すのみ）。**接続した実データ**: 店舗基本情報（店舗名/住所/最終来店/メモ）、会員カード（作成有無・貯玉残高・入金残高）、貯玉・精算管理（店内貯玉/店内再プレイ/本日精算予定、円換算含む）、交換率・貸玉情報（貸玉単価・交換率・玉単価は `Tabs.jsx` の Store detail view と**全く同じ計算式**＝`faceRent`/`faceEx`/`yenPerBall`/`exYenPerBall` を再利用、新しい数式は作っていない）、「現在の実戦設定に適用中」バッジ（`S.rentBalls`/`S.exRate` と店舗の値を実比較）。**重要な発見と対応**: モック画像の「再プレイ上限 500玉」は店舗ごとの値に見えるが、実装を確認したところ実際のアプリでは店舗単位ではなくグローバル設定 `S.chodamaReplayLimit`（既定 2500玉）だった。捏造せず、実際にその通りグローバル設定値を表示するようにした（Playwright実測で確認済み店舗はデフォルト値のため「2500玉」と表示）。**意図的に据え置き**: 店舗分析度・データ充足状況・傾向・判断ログ・「次に確認すること」など分析タブ関連は対応する実集計ロジックが存在しないため `mockStoreDetail.js` のダミー値のまま（`resolveStoreDetail` 内にコメントで明記）。編集・会員カード管理・貯玉入出金・実戦設定への適用・店舗削除の各ボタンは表示のみで保存処理は次ステップ（TODO継続）。登録店舗が0件の場合は従来通り `MOCK_STORE_DETAIL` にフォールバック。**保護**: `src/logic.js` 変更なし・読み取りもしていない（`node src/__tests__/protected-fns.mjs` の出力が `baseline.json` と完全一致）。既存の店舗一覧タップ→実データ編集画面・保存処理は無改変。**検証**: `npm run lint`=0 errors（既存警告1件のみ・無関連）、`npm run build`=成功。Playwright+Chromium（390×844px）で実店舗（ダイナム愛媛北条店）の会員カードを作成→貯玉残高3,400玉・入金残高1,200円を保存→店舗詳細プレビューを開き、概要タブ（店内貯玉3,400玉/約13,600円、再プレイ上限2500玉、適用中バッジ）と設定タブ（店舗名・住所・会員カード最終残高3,400玉/入金残高1,200円）の両方に正しく反映されることを実描画で確認。**保留・次ステップ**: 分析タブの実データ接続（archivesベースの店舗別集計）、会員カード/貯玉入出金・店舗編集・店舗削除の実際の保存処理、共有ボタンの実装。**前回までの経緯は下記**。）

最終更新: 2026-07-11（**店舗詳細画面への遷移導線を2箇所に追加**（ブランチ `claude/new-session-48im8n`）。**目的**: 前回追加した店舗詳細画面（概要/分析/設定、下記エントリ参照）が `currentMode==="storeDetail"` を直接セットしない限りどこからも開けなかったため、実際に画面を開ける導線を追加（着手前にユーザーへ優先度・配置場所を方針確認済み：①遷移導線の追加を優先、②配置場所は設定タブとホームの両方）。**変更ファイル**: `src/App.jsx`（`SettingsTab` に `onOpenStoreDetail` prop を追加。渡された店舗の `id` を `storeDetailId` にセットして `currentMode` を `"storeDetail"` へ切替）、`src/components/Tabs.jsx`（`SettingsTab` の店舗一覧の各行を `<button>` 単体から `<div>` ラッパー+2ボタン構成に変更。既存の行タップ（`setSelectedStore`、実データ編集画面を開く）は無改変、新規に「店舗詳細プレビューを見る ›」という別の細いボタンを行の下に追加し `onOpenStoreDetail(st)` を呼ぶ）、`src/components/home/HomeDashboard.jsx`（新規 `StoreDetailPreviewCard` コンポーネントを追加し、「差玉解析」カードの直下に表示。ボタン押下で既存の汎用ブリッジ `S.setTab("storeDetail")` を呼ぶだけで済んだため `HomeDashboard` への prop 追加は不要）。**現状の制約**: 遷移時に渡る店舗情報はまだ `storeDetailId`（店舗の `id` 文字列）のみで、`StoreDetail.jsx` 側は相変わらず `MOCK_STORE_DETAIL` 固定表示（`storeId` は未使用、TODO継続）。会員カード・貯玉・交換率の実データ接続、店舗削除の実処理は次ステップのまま。**保護**: `src/logic.js` 変更なし・`node src/__tests__/protected-fns.mjs` の出力が `baseline.json` と完全一致。既存の店舗一覧タップ→実データ編集画面（`selectedStore` ビュー）の挙動・レイアウトは無変更（Playwrightで確認）。**操作ステップへの影響**: 既存フロー（店舗一覧タップで編集画面を開く）はタップ数・挙動とも不変。新規追加は「店舗詳細プレビューを見る」ボタン（設定タブの店舗一覧の各行）と「店舗詳細プレビューを見る」カード（ホーム画面）の2つの新規導線のみ。**検証**: `npm run lint`=0 errors（既存警告1件のみ・無関連）、`npm run build`=成功。Playwright+Chromium（390×844px）で①ホーム→店舗詳細プレビューカードのタップ、②設定→店舗検索・登録→各行の「店舗詳細プレビューを見る」タップ、③既存の店舗名タップ→実データ編集画面、の3経路すべてが期待通り遷移することを実描画で確認。**保留・次ステップ**: `storeId` に基づく実店舗解決（現状は店舗一覧のどれをタップしても同じダミー内容が表示される）、会員カード/貯玉/交換率の実データ接続・保存、店舗削除の実処理、共有ボタンの実装。**前回までの経緯は下記**。）

最終更新: 2026-07-11（**店舗詳細画面（概要/分析/設定の3タブ）を新規追加（見た目優先プロトタイプ）**（ブランチ `claude/new-session-48im8n`）。**目的**: 添付モック3枚（iPhone 390px想定）に基づき、店舗ごとの分析度・貯玉状況・会員カード・交換率設定などを1画面で確認できる「店舗詳細」画面を新規作成。**新規ファイル**: `src/pages/StoreDetail.jsx`（ヘッダー+ストアヘッダーカード+セグメントタブのシェル）、`src/components/store/StoreOverviewTab.jsx`（概要）、`src/components/store/StoreAnalysisTab.jsx`（分析）、`src/components/store/StoreSettingsTab.jsx`（設定）、`src/components/store/storeDetailShared.jsx`（共用UIパーツ：`SectionCard`/`SectionHeader`/`Badge`/`StatTile`/`MiniBarSpark`/`WarningCard`）、`src/data/mockStoreDetail.js`（ダミーデータ1店舗分、`MOCK_STORE_DETAIL`）。**ルーティング**: このアプリは react-router 等を導入しておらず、画面切替は `App.jsx` の `currentMode`（状態文字列）で統一されているため、要求された「`/stores/:storeId`」は実URLではなく既存方式に合わせて `currentMode === "storeDetail"` + `storeDetailId` state（`App.jsx:96-99` 新規、`App.jsx:1167-1176` 新規分岐）として実装（着手前にユーザーへ方針確認済み、新規ライブラリ追加なし）。**既存機能との関係**: `SettingsTab`（`Tabs.jsx:9751〜`）内には会員カード・貯玉・交換率を扱う実データ版の店舗詳細機能が既に存在するが、本画面はそれとは完全に独立した見た目確認用プロトタイプであり、`Tabs.jsx` は一切変更していない（着手前にユーザーへ方針確認済み）。**現状の制約（すべて意図的）**: ①データは `mockStoreDetail.js` の固定ダミーのみ、`storeId` props は未使用（`StoreDetail.jsx` に TODO コメントあり）。②画面への遷移導線（店舗一覧等からの起動）は未実装。`App.jsx` に `setStoreDetailId(id) → setCurrentMode("storeDetail")` を呼ぶ想定のコメントのみ残置。③編集・会員カード管理・貯玉入出金・実戦設定への適用・機種検索的な各ボタンは見た目のみで保存処理は未接続（TODO コメントあり）。④店舗削除は確認ダイアログ（インライン、キャンセル/削除する）のみ実装、実際の削除処理は `StoreSettingsTab.jsx` 内に TODO コメントで明記し未実装。**保護**: `src/logic.js` 変更なし（`node src/__tests__/protected-fns.mjs` の出力が `baseline.json` と完全一致）。`evDecision.js` / `rotRows` / `jpLog` 等の保存データ構造も不変。既存の `SettingsTab`・`ModeTabBar`・他モードの画面は無改変。**操作ステップへの影響**: 既存フローへの影響なし（新規モードの追加のみ、遷移導線は次ステップで別途実装）。**検証**: `npm run lint`=0 errors（既存 App.jsx 警告1件のみ・無関連）。`npm run build`=成功。Playwright+Chromium（390×844px）で概要/分析/設定の3タブを実描画確認、タブ間のスクロール位置保持（各タブを独立 `overflow-y-auto` + `display:none/block` で維持）を実測確認、削除確認ダイアログの表示切り替えを確認。**保留・次ステップ**: 店舗一覧・店舗検索からの遷移導線、`storeId` に基づく実店舗解決、会員カード/貯玉/交換率の実データ接続・保存、店舗削除の実処理、共有ボタンの実装。**前回までの経緯は下記**。）

最終更新: 2026-07-10（**分析画面カレンダーの横スワイプ中に画面がブレる不具合を修正**（ブランチ `claude/calendar-swipe-stability-ivr5mz`）。**症状**: 分析画面（月別）でカレンダー領域を横スワイプして月送りしようとすると、スワイプ中に画面が固定されず横に引っ張られて戻るような「ブレ」が発生する。**原因**: 月送りスワイプを検知する `<main>`（`overflow-y-auto` の縦スクロール領域、`onTouchStart`/`onTouchEnd` で `dx`/`dy` を比較し `goPeriod` を発火）に `touch-action` の指定がなく、ブラウザが横方向の触操作を「未確定のパン」として解釈し、スクロール可能な要素が存在しない横方向に対して弾性バウンス（rubber-band）の視覚効果が発生していた。**修正**: `src/components/analysis/AnalysisDashboard.jsx` の該当 `<main>` に Tailwind の `touch-pan-y` ユーティリティ（`touch-action: pan-y`）を追加。縦スクロールのみブラウザのネイティブジェスチャーに委ね、横方向のタッチはブラウザの解釈対象から外して即座に既存JS（`onSwipeStart`/`onSwipeEnd`）へ渡すようにした。**変更ファイル**: `src/components/analysis/AnalysisDashboard.jsx`（CSSクラス1箇所追加のみ、+3/-0相当）。**非変更**: `src/logic.js`、計算ロジック、`evDecision.js`、保存データ構造、スワイプ判定の閾値・`goPeriod`/`onSwipeStart`/`onSwipeEnd` のロジック自体は不変（原因はCSS側のみ）。**検証**: `npm run lint`=0 errors（既存 App.jsx 警告1件のみ・無関連）、`npm run build`=成功。Playwright+Chromium実機相当（393px幅・`hasTouch`/`isMobile`）でCDP `Input.dispatchTouchEvent` により touchStart→touchMove×8→touchEnd を再現し、`main` 要素の `getComputedStyle().touchAction` が `pan-y` になっていること、スワイプ後に月ラベルが `2026年5月`→`2026年6月` へ正しく遷移すること、コンソールエラーが発生しないことを確認。**注**: 弾性バウンスはiOS Safari(WebKit)特有の視覚挙動のため、ヘッドレスChromiumでは同じ「ブレ」の見た目自体は再現されないが、原因箇所（`touch-action` 未指定）と標準的な対処（`touch-action: pan-y`）は確認済み。実機での最終確認は未実施。**操作ステップ数の変化なし**。**前回までの経緯は下記**。）

最終更新: 2026-07-06（**大当たり後フローのEVマイナス・回転率激変・持ち玉端数の使い切り不能 3バグ修正**（ブランチ `claude/ev-ball-count-bugs-r0avje`）。**変更ファイル**: `src/ballConsumption.js` / `src/components/Tabs.jsx` / `src/__tests__/ball-consumption.mjs`（`src/logic.js`・`baseline.json`・`evDecision.js` は不変）。**バグ①②（EVマイナス・予測回転率が4〜5回/Kに沈む）**: `reconcileSegmentConsumption` の瞬間当たり区間（回転入力なしで当たった区間）のグロス推定が「回転率下限5回/K＝最大50玉/回転」を上限にしており、19回転の当たりで950玉・2回転で100玉の幻の消費を計上（スクリーンショットの 予測回転率4.0/5.0回/K・実質投資4,428円をシミュレーションで完全再現）。修正: 上限を「想定回転率（呼び出し元から `expectedRate`＝理論ボーダーを受領、無ければ15回/K、5〜30にクランプ）での消費玉＋上皿残玉（新オプション `trayBalls`）」に変更し、区間ウィンドウ内の現金投資（プッシュ補正等）で説明できる回転数は玉消費から除外（現金・玉の二重計上防止）。全回転が現金で説明できる場合も最低1玉/行を書き込み `deriveFromRows` の「未設定→250玉」フォールバック発動を防止。呼び出し元 `handleWizardComplete` が `trayBalls: tray` と `expectedRate:（theoreticalBorder→border）` を渡す。**バグ②残り（大当たり後スタートの0入力が黙って破棄）**: スタート回転数プロンプトの `val > 0` ガードを「空欄以外の0以上」に変更。カウンタが0リセットされる台で0を記録でき、次の1K入力が直前当たりの古い累計と比較され逆行confirmになる混乱を解消。**バグ③（持ち玉端数が使い切れない）**: `decide()` の自動切替を「残高<250で即現金」→「残玉が1玉以上あれば端数を全量消費（`ballsConsumed=残玉`、Kは玉数比で正確に計上）し、使い切った次の入力から現金へ自動復帰」に変更。貯玉も同挙動。**検証**: `node src/__tests__/protected-fns.mjs` が `baseline.json` と完全一致（logic.js不変）、`ball-consumption.mjs` 旧15+新6=21件PASS（ケースI/J/K追加: IMG_0208/IMG_0205再現と残高0フォールバック防止）、`evDecision.test.mjs` 5/5 PASS、`npm run lint` 0 errors（既存warning 1件のみ）、`npm run build` 成功。Playwright+Chromium実機フロー検証（393px幅）: 瞬間当たり+19回転+プッシュ1000円で予測回転率16.3回/K・実質投資膨張なし、大当たり後スタート0記録→次の1K入力+20回が逆行ダイアログなしで記録、持ち玉1,640玉→250×6消費→端数140玉を7回目で使い切り（持ち玉—表示・実質投資+560円）→8回目から総投資+1,000円の現金復帰を確認。**操作ステップ数の変化なし**。**前回までの経緯は下記**。）

前回更新: 2026-06-26（**収支分析「月別」を新モック（ミニチャート/インサイト/グロー選択）へ完全再現**（ブランチ `claude/mobile-layout-adjustments-g897fk`）。**変更ファイル**: `src/components/analysis/AnalysisDashboard.jsx`（UIのみ）。**UI変更内容**: (1) **ヘッダー再設計**: 左にブランド「PachiTracker / 分析 / 判断支援」（英字表記は既存 ShareCard と同様の世界観）、中央に期間ラベル＋前後送り `‹ 2026年6月 ›`（`goPeriod` 呼び出し、年別は±12ヶ月・通算は無効）、右端にハンバーガー（プルダウンで月別/年別/通算/分析+ 切替）。カレンダーの横スワイプ月送りも併存。(2) **KPIカードにミニチャート追加**: 各カードを アイコンチップ＋ラベル/値＋右側ミニチャート の横並びに。月収支/期待値/差/時給は `Sparkline`（既存 `trend` から描画）、勝率は `MiniDonut`、稼働日数は `MiniBars`（`dayMap` から）。いずれも軽量インラインSVGの装飾で、計算ロジックには未介入・データ不足時は空枠。(3) **月間インサイトバナー新設**（`InsightBanner`：電球＋所見＋シェブロン）。所見テキストは demo（記録ゼロ時プレビュー）のみサンプル文「中盤のプラスを活かしきれず…」を表示し、本番は将来連携予定の中立文（ダミー解析の本番常駐を回避）。(4) **日別ヒートマップを角丸独立セル＋グロー選択へ刷新**: `gap-px` 罫線方式をやめ `gap-1` の角丸セルに。日付色は稼働日＝損益色/未稼働＝曜日色。選択日(19)はシアンの `box-shadow` グロー枠（カードの `overflow-hidden` を外しグローを表示）。(5) **選択日詳細を5指標化**: 日付＋実収支バッジ、実収支/期待値/差/時給/稼働時間の5列、メモ入力プレースホルダー（将来連携予定）、記録を編集（CalendarTab遷移）をスリム残置。初期選択日を 18→19 に変更（モック準拠）。**スコープ厳守**: `src/logic.js`／計算式／期待値・収支計算／保存データ構造／`evDecision.js` は不変。`package.json`/lockfile 変更なし。日本語ラベルのみ（ブランド表記 PachiTracker は既存踏襲）。**確認**: `git diff -- src/logic.js`=差分なし、`npm run lint`=0 errors（既存 App.jsx:364 warning 1件のみ）、`npm run build`=成功。iPhone幅393pxで横はみ出し0件、KPIミニチャート/カレンダーグロー/詳細5指標の表示を実機相当スクショで確認。**留意**: KPIミニチャート・インサイトは装飾/プレビュー目的。実データ時の時給・稼働時間・機種等は日別集計（dayMap）に未連携のため 0/「—」表示（将来連携予定）。**前回までの経緯は下記**。）

最終更新: 2026-06-26（**収支分析「月別」ダッシュボードを添付モック準拠へ刷新＋ヘッダー操作系を再設計**（ブランチ `claude/mobile-layout-adjustments-g897fk`）。**変更ファイル**: `src/components/analysis/AnalysisDashboard.jsx`、`src/index.css`、`docs/HANDOVER.md`（UIのみ）。**UI変更内容**: (1) **月間サマリー**を2列×3段の横並びカードへ（左に符号連動色のアイコンチップ＝プラス:シアン/マイナス:レッド/勝率:シアン/稼働日数:ブルー、右にラベル＋値）。アイコンは 月収支:財布 / 期待値:折れ線 / 差:天秤 / 勝率:標的 / 稼働日数:カレンダー / 時給:時計。**「月間サマリー」見出しテキストは削除**（視認性優先）。(2) **日別ヒートマップ**見出しにカレンダーアイコンを追加。グリッドは `year`/`month` から `blanks`/`count` を算出（2026-06は月=1日始まり・30日で添付どおり）。選択日(18)は `ring-inset` シアン枠。(3) **選択日詳細**を実収支/期待値（2列）＋区切り線＋機種名/時間/時給（3列）の構成へ刷新。メモ入力プレースホルダーと「詳細を見る」展開（店舗/回転率/投資/回収）は削除し、`記録を編集`（CalendarTab遷移）はスリムなボタンで残置。(4) **ヘッダー操作系を再設計**: 旧ヘッダーの「収支分析」タイトル・「カレンダー/分析+」上位タブ・月別/年別/通算サブタブ・前回追加した右下FAB・月送り `< >` ボタンをすべて廃止。代わりに**ヘッダー右端のハンバーガー（`Menu`）→プルダウン（`HeaderMenu`）**で 月別/年別/通算/分析+ を切替（現在表示にチェック）。ヘッダー左には現在期間ラベル（例「2026年6月」）。(5) **月送りはカレンダー（コンテンツ領域）の横スワイプ/フリック**で実装（左=翌月 / 右=前月、年別は±12ヶ月、通算は移動なし）。縦スクロールを阻害しないよう「横移動量>48px かつ 横>縦×1.2」のときのみ反応。(6) **高級感のあるスライドアニメーション**を `src/index.css` に追加（`.month-pane-next/prev/fade` ＋ `@keyframes monthPaneNext/Prev/Fade`、`cubic-bezier(.22,.61,.36,1)` で横スライド＋微フェード＋微拡縮）。プルダウンは `.hdr-menu-pop` でポップ表示。月送り時は `key={periodTab-monthOffset}` の再マウントで向き別アニメを再生。**操作ステップ**: 期間切替=「ハンバーガー→項目選択」、月送り=「カレンダーをフリック」。**スコープ厳守**: `src/logic.js`／計算式／期待値・収支計算／保存データ構造／`evDecision.js` は不変。`package.json`/lockfile 変更なし。日本語ラベルのみ。**確認**: `git diff -- src/logic.js`=差分なし、`npm run lint`=0 errors（既存 App.jsx:364 warning 1件のみ・無関連）、`npm run build`=成功。iPhone幅393pxで横はみ出し0件、スワイプで 2026年6月→7月 への遷移を確認。**残課題/留意**: (a) 添付モックには `記録を編集` ボタンが無いが、日別記録編集（CalendarTab）への唯一の導線のため機能保持目的でスリムに残置（不要なら撤去可・要判断）。(b) デモ（記録ゼロ時）の `DEMO_DAYS` は日付キーのみで月非依存のため、月をスワイプしても同じデモ収支が表示される（プレビュー専用。実データは月でフィルタ済みのため問題なし）。(c) 年別の年送りも横スワイプで可能（`< >` 廃止の代替）。）

最終更新: 2026-06-22（**大当たり終了後の予測回転率表示を修正**）。**症状**: 大当たり終了直後、「予測回転率」が実測値とかけ離れた値（例: 37.5回/K）へ跳ねる。**原因**: `KeyMetrics.jsx` が予測回転率にも `effectiveStart1K` / `start1KCorrected` を優先していた。大当たり登録時の上皿残玉補正で補正後投資額の分母が小さくなるため、終了直後ほど補正回転率が大きく表示されていた。大当たり後スタート回転数（例: 55回転）の通常回転への誤加算はなかった。**修正**: 予測回転率専用の `getPredictedSpinRate` を追加し、通常は実測値 `start1K` を表示。旧データなど `start1K` がない場合のみ補正値へフォールバックする。**変更ファイル**: `src/components/decision/KeyMetrics.jsx`、`src/components/decision/metricSelectors.js`、`src/components/decision/__tests__/metricSelectors.test.mjs`、`docs/HANDOVER.md`。**非変更**: `src/logic.js`、計算ロジック、保存データ構造、大当たり後スタート行の処理、補正後EV/K・判定ロジック。**検証**: metricSelectors 3 passed、evDecision 5 passed、ball-consumption 11 passed、protected-fns baseline一致、lint 0 errors（既存warning 1件）、build成功。）

最終更新: 2026-06-21（**収支分析「月別」画面を添付モックアップ風に刷新**（ブランチ `claude/pachitracker-analysis-redesign-hmfjgd`）。**変更ファイル**: `src/components/analysis/AnalysisDashboard.jsx` のみ（UIのみ）。**目的**: 大型の月間収支ヒーローカードが縦に重く、要素のはみ出し・縦横バランス崩れが続いていたため、控えめで画面内に収まるプロ向け収支ダッシュボードへUI刷新。**主な変更**: (1) 月別トップの `SummaryHero`（42px大型数字カード）を廃止し、新規 `MonthKpis`（月間収支/期待値/勝率/稼働日数の4KPIカード横並び、`grid-cols-4 gap-1.5`、各カード `min-w-0`＋`whitespace-nowrap`＋`text-[clamp(13px,3.4vw,15px)]` で縦幅圧縮）に置換。`SummaryHero` は年別/通算では従来通り使用（移動のみ）。(2) `CalendarPanel`/`CalendarCell` を表形式に刷新：見出し「日別収支」＋右側にコンパクト凡例（プラス/±0/マイナス）、曜日見出し（日=赤系・土=青系）、`grid-cols-7 gap-px` ＋親背景で細い罫線、各セル `aspect-[1/0.92]`・薄い緑/赤ヒート・日曜赤/土曜青の日付色・選択日(18)は `ring-inset` シアン枠。固定30日→`year`/`month` から `blanks`/`count` を算出する正しいグリッドに修正（旧 `blanks=1`・30日固定を解消）。(3) `DayDetail` をモック準拠の縦幅圧縮レイアウトに刷新：カレンダーアクセント＋日付、実収支/期待値の2カラム、メモ入力導線（保存層未接続のプレースホルダー＝将来連携予定）。既存の店舗/機種/回転率/稼働時間/投資/回収は削除せず「詳細を見る」展開内に保持。(4) 月別のデモ表示値をモック準拠に更新（`DEMO_DAYS`＝1日 -2,000 / 5日 +10,000 / 14日 -500 / 15日 +943 / 17日 +6,937 / 18日 0(選択) / 19日 +3,486、月別KPIデモ値＝月間収支 -11,704円 / 期待値 +2,934円 / 勝率 67% / 稼働日数 8日）。これらは記録ゼロ時のプレビュー専用値で、年別/通算の `SummaryHero` デモ値は不変。**下部ナビ**: グローバルの `ModeTabBar`（ホーム/台選び/記録開始+/分析/設定）が既にモック準拠のため新規追加せず。本文は App の `<main>` 下部パディング＋各画面 `pb-10` で被り回避済み。**操作ステップ**: 増減なし（タップ数不変）。**スコープ厳守**: `src/logic.js`／計算式／保存データ構造／`evDecision.js` は不変。保護関数テスト `node src/__tests__/protected-fns.mjs` は `baseline.json` と完全一致。**確認**: `npm run lint`=0エラー（既存 App.jsx:363 警告1件のみ・無関連）、`npm run build`=成功。package.json/lockfile 変更なし。**横はみ出し対策**: max-w-[430px]＋px-4 内に4KPI/7列カレンダーとも `min-w-0`・`whitespace-nowrap`・`tabular-nums`・`tracking` 詰めで収容（390px幅で算出上の溢れなし）。**未取得**: 当環境にブラウザ実行系が無く実機スクショ取得不可（幅計算で収まりを確認）。）

最終更新: 2026-06-21（**カレンダー実額が狭いiPhone幅で横にはみ出す/見切れる問題を修正**（ブランチ `claude/pachitracker-analysis-iphone-ui-s4tnz6`）。**変更ファイル**: `src/components/analysis/AnalysisDashboard.jsx` のみ（UIのみ・+9/-8）。**真因**: ユーザー報告「実機ではみ出す」は**縦スクロールではなく横方向**だった（ユーザー確認で確定）。カレンダーを実額表示（「k」廃止）にした結果、`+10,000` 等の数字が7列セル幅に対して大きすぎ、画面幅の狭いiPhone（375〜390px）で右端がクリップ＝文字見切れ。試算: 旧構成(container px-4 / panel p-3 / grid gap-6px / cell px-0.5 / 金額9px)では375px幅でセル内容領域≈36px<数字幅≈38pxで溢れ。**修正**: (a) `CalendarCell` セル余白 `px-0.5→px-0`＋`overflow-hidden` 付与、金額 `text-[9px]→text-[8px]` ＋ `tabular-nums` ＋ `tracking-[-.04em]`、金額ラッパに `w-full px-0.5`、日付に `px-1`。(b) `CalendarPanel` パネル余白 `p-3→p-2.5`、グリッド＆曜日ヘッダーの `gap-[6px]→gap-1(4px)`（ヘッダーにもgap付与で列整列）。これで375px幅でもセル内容領域≈39px>数字幅≈34pxと余裕を確保し、393px(16 Pro)では更に余裕。**スコープ**: `logic.js`/計算式/保存データ構造/evDecision.js は不変。`npm run lint`=0エラー（既存 App.jsx:363 警告1件のみ）、`npm run build`=成功。package.json/lockfile変更なし。**補足**: 前回までの縦スクロール抜本改修（ネストflex）と併せ、月別画面は縦も横も画面内に収まる。**注意（PWAキャッシュ）**: デプロイ後に実機で旧UIが残る場合はアプリ再読込でSW更新が必要。**未取得**: 当環境にブラウザ実行系が無く実機スクショ取得不可（幅計算で収まりを確認）。**カレンダーの既知の簡略化（別件・未対応）**: `CalendarPanel` の先頭空白は `blanks=1` 固定・日数は30固定で、実月の曜日整列/日数を反映していない（既存挙動・本タスク範囲外）。）

最終更新: 2026-06-21（**収支分析ページのレイアウトを「画面内完結スクロール」構造へ抜本改修**（ブランチ `claude/pachitracker-analysis-iphone-ui-s4tnz6`）。**変更ファイル**: `src/components/analysis/AnalysisDashboard.jsx` のみ（UIのみ・+16/-12）。**背景**: 前回の `shrink-0` 対処後も実機(iPhone)で「画面からはみ出る／下部固定ナビ(ModeTabBar)と重なる／最下部(SNS共有CTA・分析+導線・選択日詳細の詳細ボタン)に届かない」状態が継続。スクリーンショットでは収まって見えるが実アプリでは破綻、という典型的な高さ/スクロールモデルの不安定（親 `App.jsx <main>`(flex:1/overflow:auto)のスクロールと、ページルートの `min-h-full`/可視オーバーフロー依存）が原因。**抜本対処**: 親mainのスクロール挙動やパーセント高さ依存をやめ、各ビューを**ネストflexの確実なスクロール構造**へ再構成。具体的には3つのreturn(月別/年別/通算=main・分析+=analyzer・カレンダー)それぞれで、(1) ルート `div` を `flex min-h-0 flex-1 flex-col overflow-hidden`（親mainをflexで隙間なく充填）、(2) 内側コンテナを `mx-auto flex min-h-0 w-full max-w-[430px] flex-1 flex-col px-4 pt-3`、(3) ヘッダー(DashboardTop)・タブ・年月切替は固定(非スクロール／年月切替に `shrink-0`)、(4) ヒーロー以下を**専用スクロール領域** `min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain pb-10` に閉じ込め。カレンダービューも同様にヘッダー固定＋`CalendarTab` を `min-h-0 flex-1 overflow-y-auto` のスクロール領域でラップ。これにより**下部ナビと重ならず・はみ出さず・最下部まで確実にスクロール**可能。`pb-[140px]` の暫定下余白は廃し、スクロール領域の `pb-10` で固定ナビ高さ分をクリア。**スコープ**: `App.jsx` 等の共有レイアウトは無改変（分析ページ内で完結）。`logic.js`/計算式/保存データ構造/evDecision.js は不変（`git diff --quiet src/logic.js`=不変）。`npm run lint`=0エラー（既存 App.jsx:363 警告1件のみ）、`npm run build`=成功（既存 chunk>500kB 警告のみ）。package.json/lockfile変更なし。**注意（PWAキャッシュ）**: 本アプリはService Worker(workbox)でプリキャッシュするため、デプロイ後に実機で旧UIが残る場合はアプリ再読込/SW更新（再訪問やキャッシュクリア）が必要。**未取得**: 当環境にブラウザ実行系が無く実機スクショは取得不可。修正はネストflex(flex-1+min-h-0+overflow-y-auto)の確立されたスクロールパターンに基づく確実な是正。）

最終更新: 2026-06-21（**収支分析ページのスクロール不能を修正＋ヘッダーの共有/絞り込みボタンを排除**（ブランチ `claude/pachitracker-analysis-iphone-ui-s4tnz6`）。**変更ファイル**: `src/components/analysis/AnalysisDashboard.jsx` のみ（UIのみ・+8/-33）。**不具合1（スクロール不能）**: 分析ページ(月別/年別/通算/分析+/カレンダー)が下端まで到達できずSNS共有CTA・分析+導線が見切れていた。原因は、スクロール親 `App.jsx` の `<main>` が `display:flex; flex-direction:column; overflow:auto` のflexコンテナで、ページルート `div.analytics-terminal.min-h-full` がflex itemとして **flex-shrink:1 で画面高(viewport)まで圧縮**され、ボーダーボックスが画面高に丸められて `main` の `scrollHeight` が伸びず（＝可視オーバーフローのスクロール寄与に依存する不安定な状態）スクロールできなかった。**修正**: 3つのページルートに `shrink-0`(flex-shrink:0) を付与。これによりルートのボーダーボックスが実コンテンツ高に固定され、`main` のscrollHeightが正しく伸びて確実にスクロール可能に。`min-h-full` は維持（コンテンツが短い時は画面高までbg充填）。`App.jsx` 等の共有レイアウトは無改変（他画面非干渉）。**不具合2/要望（ヘッダー整理）**: ヘッダー右の「共有」「絞り込み」ボタンを排除し、`DashboardTop` をタイトル「収支分析」のみへ簡素化。`DashboardTop` のpropを `{periodTab,setPeriodTab}` のみに縮小し、3箇所の呼び出しから `showActions/filterOpen/setFilterOpen/onShare` を除去。**SNSカード作成はページ下部のSNS共有カード(ShareCTA)から従来どおり到達可能**。絞り込み(FilterPanel)はヘッダーの入口を撤去したため現状UI上は非表示（`FilterPanel`/filter state自体はコード保持＝将来別所へ再配置可能・lintは `varsIgnorePattern:^[A-Z_]` で未使用大文字を許容、filterOpen等は条件レンダリングで参照継続のためエラーなし）。**スコープ**: `logic.js`/計算式/保存データ構造/evDecision.js は不変（`git diff --quiet src/logic.js`=不変）。`npm run lint`=0エラー（既存 App.jsx:363 警告1件のみ）、`npm run build`=成功（既存 chunk>500kB 警告のみ）。package.json/lockfile変更なし。**未取得**: スクリーンショットは当環境にブラウザ実行系が無く取得不可。スクロール修正はflexのscrollHeight計算（shrink-0でボーダーボックス=実高）に基づく確実な是正。）

最終更新: 2026-06-20（**収支分析「月別」をiPhone専用に最適化（見た目優先プロトタイプ）**（ブランチ `claude/pachitracker-analysis-iphone-ui-s4tnz6`）。前回の「収支報告ページ(90点版)」(PR#300マージ済み)を、提供モックアップ3枚に合わせてさらに調整。**変更ファイル**: `src/components/analysis/AnalysisDashboard.jsx` のみ（UIのみ・+86/-41）。**主な変更**: (a) **カレンダーの「k」表記を全廃**＝旧 `shortMoney`（`+5.4k` 形式）ヘルパーを削除し、セルは `signed()` の実額表示（例 `+5,406` / `-15,500`・カンマ区切り）に変更。フォントは収支9px(font-mono/tracking-[-.02em])・日付11px、セル `min-h-[58px]→[56px]`・`p-1→px-0.5 py-1` でiPhone16Pro幅(393px・7列)に実額が収まるよう調整。**0円の日は金額非表示**（`hasAmount = row && actual!==0`・色のみで把握）。プラス期待値の青ドットも撤去し、セル内を「日付＋収支金額のみ」へ簡素化（モック「期待値はセルに入れない」準拠）。(b) **月間収支サマリーの補助3指標**を `期待値／差異／稼働` → **`期待値／勝率／稼働日数`** へ変更（モック準拠。勝率=`summary.winRate` 四捨五入・%表示、稼働日数=`summary.days`。差異(=実収支−期待値)の表示を撤去）。金額42pxヒーローは不変。(c) **SNS共有CTA** を「SNSで成果をシェアしよう！」→ **左に共有アイコンボックス＋『今月の結果を共有／月間収支カードをSNSに投稿できます』＋右に『SNSカード作成』ボタン(h-11=44px)** へ刷新（モック準拠）。`title`/`subtitle` をprop化し、年別/通算では `title="成果を共有"` を渡す。(d) **SNSカード(ShareCardモーダル)を上品な明色カードへ全面再設計**＝旧ダークカードの **「期待値ランク A-」「🏆データ派プレイヤー」を撤去**（指示通りグレード/称号なし）。新カードは `#fbfbf9` 明色・serif見出し `YYYY年M月`・`実質収支` ラベル・34pxの実質収支(プラス緑#1a8f4c/マイナス赤#d6394c)・3分割の `期待値/勝率/稼働`・**ミニカレンダー(新規 `ShareMiniCalendar`・7列固定・実額・5段ヒートの淡色)**・フッター `PachiTracker`。`year/month/actual/ev/winRate/days/dayMap` をpropで受け取り実データ反映（機種/店舗画像など著作権素材は不使用・テキストとレイアウトのみ）。**スコープ厳守**: 年別・通算・分析+・カレンダーの構成は基本不変（ShareCTAの文言propのみ年別/通算へ波及）。`logic.js`/計算式/保存データ構造/evDecision.js は不変。**保護検証**: `git diff --quiet src/logic.js`=不変。`npm run lint`=0エラー（既存 App.jsx:363 警告1件のみ・本変更外）、`npm run build`=成功（既存 chunk>500kB 警告のみ）。`package.json`/lockfile 変更なし（npm install由来のlock差分は `git checkout` で復元）。**操作ステップ**: タブ/年月/日付選択/共有のタップ数は不変。**未取得**: スクリーンショットは当環境にブラウザ実行系が無く取得不可（コードレビューで max-width 430px・7列セルの実額が固定幅を超えないことを確認＝横はみ出し無し）。デモ値(archives空時: 実収支-12,130/期待値+3,120/勝率60/稼働7日)はCLAUDE.md「既存デフォルト不変更」に従いモック黒字値には合わせず維持。）

最終更新: 2026-06-20（**収支分析「月別」画面を「収支報告ページ（90点版）」へ再設計（見た目優先プロトタイプ）**（ブランチ `claude/pachitracker-analysis-redesign-a9fsvm`）。前回2026-06-19のダッシュボード化（PR#299マージ済み）をさらに軽量化し、月別トップの主役を**①月間収支②日別収支カレンダー③SNS共有**の3点へ集約。グラフ・KPI・ランキング・AIは補助扱いに降格。**変更ファイル**: `src/components/analysis/AnalysisDashboard.jsx` のみ（UIのみ）。**月別ビューの新構成（縦1列）**: ①ヘッダー②タブ③年月切替④月間収支ヒーロー⑤日別収支カレンダー⑥選択日ミニ詳細⑦SNS共有CTA⑧補助分析への導線。**主な変更**: (a) `SummaryHero` を圧縮（p-4→p-3.5・収支額48px→42px・余白短縮で約120〜130px化）。下段3分割の中央ラベルを「期待値超過」→**「差異」**（=実収支−期待値・値の意味は不変、モック表記に合わせ改称）、右を「稼働日数」→「稼働」に短縮。(b) `CalendarCell` を**2行→実質1行＋日付**に簡素化＝セル内の期待値テキスト常時表示を廃止し、**プラス期待値の日のみ右上に小さな青ドット**で示す。実収支は**短縮表記**（新ヘルパー `shortMoney`：±1,000以上を `+5.4k` 形式に・表示専用）。日付12px/収支12pxへ拡大、角丸10px・gap6px。(c) カレンダー凡例を上部5段表記から**下部の3項目（プラス／±0／マイナス）**へ移動・縮小（高さ約20px）。(d) `DayDetail` を**コンパクトなミニ詳細**化：初期表示は日付・実収支・期待値・店舗名・機種名のみ、回転率/稼働時間/投資/回収は**「詳細を見る」タップで展開**（`useState` の `open`）。**本番データではダミー店舗名/機種名/明細を出さず `—` 表示**（`isDemo` を受け取り、デモ時のみサンプル値）。日付ラベルは年月から曜日を算出（`WEEKDAYS`）。(e) 新規 `AnalysisLink`（補助分析への導線カード：「詳しく分析する／収支推移・機種別・店舗別を見る」＋「分析+を見る」ボタン→`setPeriodTab("analyzer")`）。(f) `ShareCTA` に `subtitle` propを追加し月別では「月間収支とカレンダーを画像で共有できます」を表示。(g) **月別トップから `TrendPanel`（収支推移グラフ）と `Kpis`（KPIミニカード）を撤去**＝分析+導線へ集約（年別/通算ビューでは従来どおり TrendPanel/Kpis を維持）。機種/店舗ランキングは前回PRで既に分析+へ移設済み。**スコープ厳守**: 年別・通算・分析+・カレンダーの中身は不変。`logic.js`/計算式/保存データ構造/evDecision.js は不変。**保護検証**: `git diff --quiet src/logic.js`=不変（md5 `17363072a506e101f456f42ef27baba4`）、`node src/__tests__/protected-fns.mjs` 正常終了（baseline一致）。`npm run lint`=0エラー（既存 App.jsx:363 警告1件のみ・本変更外）、`npm run build`=成功（既存 chunk>500kB 警告のみ）。`package.json`/lockfile 変更なし（npm install由来のlock差分は `git checkout` で復元）。**操作ステップ**: タブ/年月/日付選択/共有のタップ数は不変。選択日詳細は「詳細を見る」1タップで明細展開（任意）。**未取得/未対応**: スクリーンショットは当環境にブラウザ実行系が無く取得不可（コードレビューで max-width 430px・固定px幅のコンテナ超過なし＝横はみ出し無しを確認）。月別から外した収支推移グラフは分析+の `AnalyzerView`（既存チャート）で確認可能なため `TrendPanel` の分析+への複製は行わず（分析+の中身を今回触らない方針）。`ShareCard`（SNS共有モーダル出力）内の「期待値ランクA-」「🏆データ派プレイヤー」は前回同様モック対象外のため未変更。）

最終更新: 2026-06-19（**収支分析ページを「月間収支ダッシュボード（SNS共有志向）」へリデザイン（見た目優先プロトタイプ）**（ブランチ `claude/pachitracker-iphone-layout-4hcbg1`）。前回の1カラム化（下記2026-06-18エントリ・PR#295マージ済み）に続き、ユーザー提供モックアップに沿って「評価ランク・称号中心」から「**月間収支が主役**」のダッシュボードへ再設計。**変更ファイル**: `src/components/analysis/AnalysisDashboard.jsx` のみ（UIのみ）。**月別/年別/通算ビューの新構成（縦1列）**: ①ヘッダー②タブ③年月切替④**月間収支ヒーローカード**（`SummaryHero` を刷新。収支額を48pxで最大表示＋`期待値`/`期待値超過`(=実収支−期待値・旧「期待値との差」をモック表記に合わせ改称)/`稼働日数` の3指標を横3分割。**収束率バー・期待値ランク(A-)・称号(データ派プレイヤー)を全削除**）⑤日別収支カレンダー（色凡例を**カード上部**へ移動し**5段階**=濃緑+10,000/緑+1,000/灰±999/赤-1,000/濃赤-10,000に拡張。`CalendarCell` の heat 判定に `<=-10000` の濃赤段を追加）⑥選択日詳細（`DayDetail`・カレンダー直下）⑦**収支推移グラフ**（`TrendPanel`・高さ240→**220px**・タイトルを「収支推移グラフ」に・**AIからの一言フッターを削除**）⑧**KPIミニカード**（`Kpis` を `稼働日数→` 廃し **期待値/平均回転率/時給/勝率/稼働時間** に再編。`時給`=実収支÷稼働時間の表示計算を新規追加〔workHours>0時のみ・logic.js非依存の表示用〕。2列グリッド・最後の稼働時間のみ全幅は踏襲）⑨**SNS共有CTAカード**（新規 `ShareCTA`。「SNSで成果をシェアしよう！」＋「SNS用カードを作成」h-11=44px・押下で既存 `ShareCard` モーダルを開く）。**ランキングの移動**: 機種/店舗ランキング（`MachinePanel`/`StorePanel`）は月別画面の主役でないため**月別/年別/通算から外し、分析+タブ（`analyzer` ブランチ）へ移設**（削除せず移動＝CLAUDE.md「削除ではなく移動/提案」に準拠。`machines`/`stores`/`sortMode` は早期return前の本体で算出済みのためそのまま利用）。**削除コンポーネント**: `MonthlySummary`（実収支/収束率/期待値ランク/称号を含むサマリー。称号・グレード非表示の指示によりどこにも表示せず＝関数ごと削除）。**import整理**: 未使用化した `CalendarDays`/`HelpCircle`/`Trophy` を除去、`時給`アイコン用に `Wallet` を追加。**DEMO値の方針**: モックは黒字(+152,380等)だが、CLAUDE.md「既存の初期値・デフォルト値を勝手に変更しない」を優先し**既存DEMO（archives空時の赤字月: 実収支-12,130/期待値+3,120/稼働7日/平均回転率27.2/稼働5.1h）は不変更**。新規`時給`のDEMOは既存DEMOと整合する `-2,378`(=-12,130÷5.1) を表示（モックの+2,615/hには合わせていない＝実データ投入時は実値表示）。**未対応/提案**: `ShareCard`（SNS共有モーダルの出力カード）内に残る「期待値ランクA-」「🏆データ派プレイヤー」はモック対象外かつ別アーティファクトのため本回未変更（モックの主旨に合わせ将来削除を提案）。DEMOプレビューをモックの黒字値に揃えるかは要判断（指示あれば対応）。**不変**: `src/logic.js`（md5 `17363072a506e101f456f42ef27baba4` で変更前後一致）・計算式・保存データ構造・evDecision.js・package.json/lockfile（npm install由来差分は `git checkout` で復元）・`AnalyzerView`/`CalendarTab`。**操作ステップ**: タブ/年月切替/日付選択/共有の導線・タップ数は不変。検証: `npm run lint`=0エラー（既存 App.jsx:363 警告1件のみ・本変更外）/ `npm run build`=成功（既存chunk>500kB警告のみ）。**未取得**: スクリーンショットは当環境にブラウザ実行系が無く取得不可（`cdn.playwright.dev` がegress許可リスト外で403・Chromium入手不可）。iPhone16Pro幅393pxに対しコンテナ最大430px・左右16px・固定px幅がコンテナ超過しないことをコードレビューで確認し横はみ出し無し。）

最終更新: 2026-06-18（**収支分析ページ（分析タブ）をiPhone縦画面用に1カラム化（見た目優先プロトタイプ／レイアウト修正）**（ブランチ `claude/pachitracker-iphone-layout-4hcbg1`）。`AnalysisDashboard.jsx`（分析タブ＝月別/年別/通算ビュー）がPCダッシュボード風の横2カラムでiPhone画角に収まらず、文字潰れ・下部ナビ被りが発生していた問題を、**iPhone縦画面で全セクション縦1列**になるよう修正。**変更ファイル**: `src/components/analysis/AnalysisDashboard.jsx` のみ（UIのみ）。**主な修正**: ①ページコンテナを `max-w-[480px] px-3 pb-24`→`max-w-[430px] px-4 pb-[140px]`（左右16px・下部140pxで `ModeTabBar` 下部ナビ＋中央＋ボタンの被り回避。analyzer/メイン両方の return を統一）②`SummaryHero` を `grid-cols-[1.03fr_.97fr]` の2カラム→**1枚カード縦積み**（min-h-[280px]。実質収支→期待値/差→収束率→期待値ランク/称号の順）③`Kpis` を `grid-cols-5` 1行→**2列グリッド**（`grid-cols-2`、最後の稼働時間カードのみ `col-span-2`、各セル min-h-[62px]・フォント拡大）④`CalendarPanel` の `grid-cols-[1fr_124px]`（カレンダー＋右側詳細）を解体し**カレンダーは横幅100%**、`DayDetail` を**カレンダー直下の独立カード**として縦に配置（右横配置を廃止）⑤`CalendarCell` を `h-[39px]`→`min-h-[58px]`・日付/実収支/期待値のフォント拡大（潰れ対策）⑥`DayDetail` を `aside`（border-l）→独立カード化、ボタンを h-8→h-11（44px確保）⑦累積推移グラフ `TrendPanel` の高さを `h-[132px]`→`h-[240px]`⑧`TrendPanel`/`MachinePanel` の `grid-cols-[.88fr_1.12fr]`、`StorePanel`/`MonthlySummary` の `grid-cols-2` を**すべて解体し縦1列**（`main` を space-y-2→space-y-3）⑨機種/店舗ランキングは列構成（順位/名称/時間/回転率/期待値/実収支/勝率・規模）を**維持**しつつ、全幅化に合わせフォント6/7px→8/9px・列幅拡大で潰れ解消（CLAUDE.md「既存表示項目を勝手に削除しない」を優先し列削除・タップ詳細展開は不採用。全幅化で可読性は確保）。**横並び廃止**: カレンダー×詳細／グラフ×ランキング／店舗ランキング×今月サマリーは全て縦積みに統一。**不変**: `src/logic.js`（md5 `17363072a506e101f456f42ef27baba4` で変更前後一致を確認）・計算式・保存データ構造・evDecision.js・package.json/lockfile（npm install由来のlibcフィールド差分は `git checkout` で復元）。既存のDEMOデータ（archives空時の表示用・既存挙動）は不変更。**操作ステップ**: タブ/年月切替/日付選択/共有の導線・タップ数は不変（レイアウトのみ）。検証: `npm run lint`=0エラー（既存 App.jsx:363 警告1件のみ・本変更外）/ `npm run build`=成功（既存のchunk>500kB警告のみ）。**未取得**: スクリーンショットは当環境にブラウザ実行系（chromium/playwright/puppeteer）が無く取得不可（HANDOVER既報のネットワーク制限と同様）。iPhone 16 Pro幅393pxに対しコンテナ最大430px・左右16px paddingで横はみ出し無し（固定px幅がコンテナ幅を超えないことをコードレビューで確認）。）

最終更新: 2026-06-17（**島マップ管理画面を「管理専用」へ再設計（戦略マップとの役割分離）**（ブランチ `claude/island-map-redesign-qn4h0o`）。同日先行のリニューアル（本ファイル下記エントリ）で島マップ管理画面に持ち込んでいた**分析情報を全廃**し、「店舗の島構成を管理する」画面に作り直した。**役割分離**: 島マップ管理＝「管理」（島名・機種名・台番号範囲・台数・並び）、戦略マップ＝「分析」（推定回転率・確信度・期待値・着席推奨・TOP5・候補台 等）。**島マップ管理から削除した表示**: 推定回転率／確信度／良台率／密度／強ゾーン／期待値／TOP5／候補台／着席推奨（＝これらは戦略マップ専用）。**画面構成**: ①ヘッダー（戻る／島マップ管理／サブタイトル「島レイアウト管理」／使い方／新規作成）②店舗サマリー（店舗名・住所・最終更新・総島数・総台数＋複数店舗時の店舗切替）③ホール全体プレビュー＝主役・俯瞰確認専用（各島カードに機種名・台番号範囲・台数・簡易識別色のみ）④タブ3つ（**島一覧＝初期表示**／レイアウト図／変更履歴。管理画面のため初期は島一覧）⑤島一覧＝メイン・カード形式（島名・機種名・範囲・台数・編集ボタンのみ。分析値は非表示）⑥島詳細＝カードタップで展開し台番号一覧＋レイアウトプレビュー（番号のみ）⑦**編集画面＝別画面**（島名・機種名・範囲・台数＋レイアウト編集〔台追加/台削除/行追加/列追加/並び替え〕＋保存ボタン。下書き編集→「保存」で確定。保存後に戦略マップへ反映）⑧変更履歴（更新日時・変更内容・変更者）⑨クイックアクション（島追加／台番号一括編集／台数一括編集／データリセット〔赤のみ〕）。**データ構造の制約への対応**: Island スキーマ `{id,name,start,end,machineName}` は不変（hallMapSelectors の normalize が未知フィールドを除去）。そのため「台追加/台削除」＝終了番号±1、「行追加」＝列数分の台を末尾に追加、「列追加」＝**表示レイアウトの列数のみ変更（保存対象外・非永続）**、「並び替え」＝島順の上下移動（即時）として実装し、編集画面に注記を明示。「保存」は島名/機種名/開始/終了を `updateIsland` で確定。**変更ファイル**: `src/components/select/IslandMapManager.jsx`（全面書き換え）。**削除ファイル**: `src/components/select/islandMapData.js`（分析系の仮データ生成器。分析表示を廃止したため不要化）。**Tabs.jsx は先行PRで結線済みのため本回は無変更**（`IslandMapManager` をそのまま使用）。プレビュー/詳細は実体の島範囲（start/end）から台番号のみを生成し、**決定論的擬似乱数による分析仮データは生成しない**。**不変**: `src/logic.js`（baseline.json 完全一致を確認）・evDecision.js・計算式・保存データ構造（`pt_hallMaps` スキーマ不変）・**戦略マップ画面（`strategy/` 配下・無変更）**・台選びの `HallMapEditor`・package.json。**操作ステップ**: 設定→「島マップ管理」の到達導線は不変。編集は各島「編集」1タップで別画面→「保存」で確定。検証: lint 0エラー（既存 App.jsx:363 警告1件のみ・本変更外）/ build 成功 / protected-fns.mjs baseline 完全一致 / hallMapSelectors.test.mjs 11 passed・0 failed。**未対応**: 変更履歴の端末をまたぐ恒久保存・店舗実画像・「列追加（列数）」の永続化（データ構造制約のため表示専用）・「戦略マップへ即反映」は `strategyMapData.js` が現状ハードコード仮データのため文言上の将来連携。スクリーンショットは当環境がネットワーク制限でChromium取得不可のため未取得。）
...
#### 分析ページ（収支分析）の金融端末風 全面刷新（✅ 本ブランチ `claude/pachitracker-analytics-redesign-t2bs63`、2026-06-18）

- **目的**: 「機能は豊富だが何を見るべきか分かりにくい」分析ページを、期待値稼働ユーザーが毎日確認したくなる「期待値投資の成績表」へ再設計（Bloomberg × TradingView × パチンコ）。**見た目優先プロトタイプ＋表示専用セレクタの追加**であり、`logic.js`・計算式・保存データ構造には一切触れていない。
- **配色（金融端末風）**: `index.css` に `.analytics-terminal` スコープを追加（`.cal-terminal` と同方式）。背景 `#050A14` / カード `#0C1324` / アクセント シアン `#18D7FF` / 利益 `#18E07A` / 損失 `#FF5A68`。既存の `C` トークン（`--bg`/`--surface`/`--green`/`--red`/`--blue`/`--text`/`--sub`/`--border` 等）をこのスコープ内で上書きすることで、配下コンポーネントが自動的に新配色を使う。常時ダーク固定（ライト分岐なし）。グローバル CSS 変数は変更していない。
- **タブ構成は現行維持**: 月別 / 年別 / 通算 / 分析+ / カレンダー（5タブ）。
- **月別・年別・通算（`AnalysisDashboard.jsx`）**:
  - **ヒーローカード**（主役）: 「今月の実質収支」を 42px の大型表示。サブに `期待値`（シアン）と `期待値との差`。
  - **期待値ステータス**: 期待値 / 回転率 / 店舗選択 / 収支 / 総合評価 の 5 グレードカード（A+〜D）。**評価は表示用の「目安」**であり金銭計算に影響しない（`buildGrades`）。
  - **サマリー 5 カード**: 稼働日数 / 勝率 / 平均回転率 / 期待値 / 稼働時間。
  - **累積収支推移グラフ**（TradingView 風・3 本ライン）: 実収支（緑）/ 期待値（シアン）/ 差異（グレー破線）の累積。`CumulativeChart`（`AnalyticsCharts.jsx`）。
  - 既存の `機種別成績 TOP5` / `機種別ハマり分析` / `詳細指標` / `実質収支（貯玉込み）` / 絞り込みパネル / 期間ナビは**温存**（配色のみ刷新）。
- **分析+（`AnalyzerView.jsx`）を内部3タブに再編**:
  - **期待値分析**: 期待値 / 実収支 / 期待値との差 / 収束率 / 期待値・実収支の日あたり・時間あたり（8 指標）＋累積3本ライン＋ボーダー差ヒストグラム＋**AIコメント**（ルールベースの自動短評 `buildAiComment`、外部 API 不使用）。
  - **店舗分析**: ランキング（順位 / 店舗 / 回転率 / EV / 実収支、稼働日数併記）＋**店舗強度ヒートマップ**（緑=強い / 黄=普通 / 赤=弱い、色のみ）。
  - **機種分析**: ランキング（順位 / 機種 / 稼働時間 / 平均回転率 / EV / 実収支 / 勝率）＋並べ替え（収支順 / 期待値順 / 回転率順 / 稼働時間順）＋選択機種の回転率/K 推移（既存 `buildSpinRateTrend` を温存）。
- **カレンダー**: 既存 `CalendarTab`（`.cal-terminal` の証券端末風）をそのまま埋め込み。ヒートマップ（利益=緑 / 負け=赤 / 選択=黄）＋下部詳細は既存実装が要件を満たすため非改修。
- **新規ファイル（すべて純粋関数・読み取り専用 / `logic.js` 非依存）**:
  - `src/components/analysis/analyticsViewSelectors.js`: `avgSpinRate` / `avgBorderDiff` / `plusRateBorder` / `buildCumulativeTrend` / `buildSessionCumulative` / `buildEvAnalysis` / `storeAnalysis` / `machineAnalysis` / `buildGrades` / `buildAiComment`。EV・実損益などの数式は既存 `analysisSelectors` / `analyzerSelectors` の値をそのまま使い、再集計・正規化・表示整形のみ。
  - `src/components/analysis/AnalyticsCharts.jsx`: 共有チャート（`CumulativeChart` / `Legend` / `MetricCell`）。
- **保留事項**: 店舗ランキングの「設置台数区分（大型/中型/小型）」は、`archives` にも店舗マスタ（`ehimeStores.js`）にも台数情報が無いため**未実装**（ダミーデータ禁止のルールに従い、捏造せず省略）。台数データの保存層が追加されればバッジ表示は容易に拡張可能。
- **保護検証**: `npm run lint`（エラー 0・既存の App.jsx 警告 1 のみ）/ `npm run build`（成功）/ 既存 `analysisSelectors`・`analyzerSelectors` テスト 21 件パス。`src/logic.js` 不変。`package.json` 不変。
- **操作ステップへの影響**: タブ数・入力フローは不変。むしろ月別を開いた瞬間に主要指標が一覧化され、深掘りは分析+ の内部タブに集約（タップ数増加なし）。
