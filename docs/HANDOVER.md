...
最終更新: 2026-06-23（**PWAを自動更新方式へ変更し「デプロイ済みなのに端末で旧UIが残る」問題を解消**（ブランチ `claude/monthly-summary-cardless-ui-dunrto`）。**背景**: 月間サマリーのUI修正（#313/#314/#315）を main へマージし GitHub Pages へデプロイ済み（Actions 全て success）にもかかわらず、ユーザーの実機で「何も変化しない」状態が続いた。**原因調査**: (1) `main` の `AnalysisDashboard.jsx` に修正が反映されていることを確認（`px-4` 等）。(2) deploy.yml（push to main で GitHub Pages へ自動デプロイ）の対象コミット a24db1c の Actions が success であることを確認。→ ビルド・デプロイは正常。**真因**: `vite.config.js` の `VitePWA` が `registerType: 'prompt'` ＋ `clientsClaim` のみ（`skipWaiting` なし）で、新しい Service Worker が install 後 **waiting 状態で待機**し、`src/main.jsx` の更新バナーをユーザーがタップするまで有効化されない設計だった。iOSスタンドアロンPWAではこのバナーが出ない/タップされないケースが多く、旧キャッシュ（旧UI）が serve され続けていた。**修正**: (a) `vite.config.js`＝`registerType` を `'prompt'→'autoUpdate'`、`workbox` に `skipWaiting: true` を追加（`clientsClaim`/`cleanupOutdatedCaches` は維持）。生成 `dist/sw.js` に `skipWaiting`/`clientsClaim` 両方が含まれることを確認。(b) `src/main.jsx`＝タップ待ちの更新バナーUI（`showUpdateBanner`・`updateBannerShown`・関連DOM/CSS生成 約80行）を削除し、`registerSW` を autoUpdate 向けに簡素化（`onOfflineReady` と `onRegisteredSW` 内の更新チェックのみ）。起動直後・30分毎・visibilitychange復帰時・focus時に `registration.update()` を呼び、新SW検知時は autoUpdate により**自動でキャッシュ更新・リロード**される。状態は localStorage(useLS) 永続化のため自動リロードでもセッションは復元。**変更ファイル**: `vite.config.js`、`src/main.jsx`。**スコープ**: `src/logic.js`・計算式・保存データ構造・UIコンポーネントは不変（PWA更新挙動のみ）。**トレードオフ**: デプロイ直後にアプリを開く/復帰すると自動で1回リロードが入る（旧来のタップ式バナーは廃止）。実戦中の中断が問題になる場合は「安全な区切りでのみ適用」へ調整可能（要相談）。**確認**: `npm run lint`=0エラー（既存 App.jsx:364 警告1件のみ）、`npm run build`=成功、`dist/sw.js` に skipWaiting/clientsClaim 確認。**注意**: 既に旧SWが入っている端末では、本修正がデプロイされた**次回起動時に1回だけ手動リロード（またはアプリ再起動）**すれば、以降は自動更新が効く。）

最終更新: 2026-06-23（**収支分析「月別」画面の月間サマリーをカードなしUIへ刷新**（ブランチ `claude/monthly-summary-cardless-ui-dunrto`）。**背景**: 月別トップの4KPIカード（`MonthKpis`＝`grid-cols-4` のアイコンチップ付き縦長カード横並び）がスマホ幅で潰れて窮屈に見え、上部カード群が主張しすぎてカレンダーが主役に見えにくかった。**変更ファイル**: `src/components/analysis/AnalysisDashboard.jsx` のみ（UIのみ・+31/-23）。**変更内容**: `MonthKpis` をカード形式から**カードなし・文字ベースの2列2段レイアウト**へ全面書き換え。(1) 角丸カード・アイコンチップ・グラデ背景・影をすべて廃止し、`<section>` を上下の薄い横罫線（`border-y border-white/[0.14]`・`px-1.5 py-3.5`）だけで区切る要約領域に変更。(2) 上段＝月間収支／期待値（主役・`clamp(22px,6.6vw,28px)`）、下段＝勝率／稼働日数（補助・`clamp(18px,5vw,22px)`）の2行2列。各行は `grid grid-cols-2`、右列に中央縦線 `border-l border-white/[0.12] pl-5`、上下段の間に薄い横線 `border-t border-white/[0.1]`。(3) 色は既存テーマを流用＝月間収支は `moneyClass(actual)`（プラス緑#25D366／マイナス赤#FF5B6E・**符号で色が変わる既存挙動を維持**）、期待値はシアン #16C8FF、勝率・稼働日数は白、ラベルは既存ミュート #8090aa。値は `whitespace-nowrap`＋`tabular-nums`＋`tracking` 詰めで折り返さず、390/430px幅とも算出上はみ出しなし（各列内容領域≈152px>「-11,704円」≈121px）。**新規色・新規ライブラリ追加なし**（既存の `signed`/`moneyClass`/`#16C8FF` を流用）。`Wallet`/`LineChartIcon`/`Target`/`Clock3` は `Kpis` で引き続き使用のためimport不変。**スコープ厳守**: `src/logic.js`（`git diff --quiet` で変更なし確認）／計算式／収支・期待値・勝率・稼働日数の算出元（`actual`/`ev`/`winRate`/`days` の値は不変）／日別収支カレンダー（`CalendarPanel`）／タブ構成／下部ナビ／記録開始ボタン／SNS共有／詳細分析は一切変更なし。デモ表示値（記録ゼロ時のプレビュー＝月間収支-11,704円/期待値+2,934円/勝率67%/稼働8日）も `App` の `useState`/`isDemo` 由来で不変。**操作ステップ**: タップ数不変（表示レイアウトのみ）。**確認**: `npm run lint`=0エラー（既存 App.jsx:364 警告1件のみ・無関連）、`npm run build`=成功（既存 chunk>500kB 警告のみ）。`package.json`/lockfile 変更なし（npm install由来のlock差分は `git checkout` で復元）。**未取得**: 当環境にブラウザ実行系が無く実機スクショは取得不可（幅計算・コードレビューで横はみ出し無し・非カード見えを確認）。）
