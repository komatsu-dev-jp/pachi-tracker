...
最終更新: 2026-05-21（大当たりタブに「現在持玉 / 現在評価 / 1Rあたりの出球」の3ヒーローカード・詳細トグル・チェーン直下「+ データを追加」ボタン・「今回の入力まとめ（未確定）」セクション・ゴミ箱アイコン付き削除ボタンを追加。`logic.js` / `baseline.json` 不変・見た目優先プロトタイプ）
