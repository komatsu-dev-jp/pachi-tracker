...
- 正式な機種名は別名より優先し、複数型式に一致する曖昧な別名は推測で選びません。
- ボーダー未登録機種へ18回/kなどの仮値を入れず、「ボーダー未設定」として予測を停止します。
- 全マスターについて、2,200回転の収支標準偏差を変えても判断用下限が変わらないことを回帰テストします。
- 4日・投入47,000玉の共通監査条件では、分析可能な全機種について判断用下限が10回/k未満へ崩れないことを確認します。
